...
def json_to_svg():
    ...
    for map_id in map_ids:
        ...
        file_name = 'map'+map_id+'_graph.json'
        try:
        ...
        for rxn in rxns:
            try:
                rxn_svg = '    <rect rxns="'+','.join(rxn['rxns'])+'" x="'+str(rxn['x']-23)+'" y="'+str(rxn['y']-9)+'" width="'+str(rxn['w']+2)+'" height="'+str(rxn['h']+1)+'" stroke="black" fill="white"></rect>\n'
            except:
                print "ERROR for reaction: ", rxn
                continue


            try:
                text_svg = '    <text x="'+str(rxn['x']-20)+'" y="'+str(rxn['y']+4)+'" font-size="9" stroke="grey" fill="black" >'+rxn['name']+'</text>\n'
            except:
                print "ERROR for reaction: ", rxn
                continue                

            svg_lines.append('<g>\n')
            svg_lines.append(rxn_svg)
            svg_lines.append(text_svg)
            svg_lines.append('</g>\n')

        svg_lines.append('</svg>')

        f = open('map'+map_id+'_graph.svg', 'w')
