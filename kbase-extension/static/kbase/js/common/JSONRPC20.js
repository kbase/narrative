                ...
                .catch((error) => {
                    ...
                    if (error instanceof DOMException && error.name === 'AbortError') {
                        ...
                        if (elapsed >= timeout) {
                            // probably a timeout.
                            throw new Error(
                                `Request canceled - probably timed out after ${elapsed}ms with timeout of ${timeout}ms`
                            );
                        } else {
                            // perhaps still could be, given unknowns about the precise timing of setTimeout, but chances are low.
                            throw new Error(
                                `Request canceled - but elapsed time ${elapsed}ms does not exceed timeout of ${timeout}ms`
                            );
                        }
                    }
                    ...
                    return response.text().then((textResponse) => {
                        try {
                            ...
                            if (response.status !== 200) {
                                // should never occur.
                                console.error(
                                    `Non-200 status ${response.status} but not error response`,
                                    response.status,
                                    jsonrpcResponse
                                );
                                throw new Error(
                            ...
                            if (jsonrpcResponse.id !== id) {
                                console.warn(
                                    `Id in request ${id} does not match id in response ${jsonrpcResponse.id}`
                                );
                            }
                            if (!jsonrpcResponse.jsonrpc) {
                                console.error(
                                    '"jsonrpc" property missing in response',
                                    jsonrpcResponse
                                );
                            ...
                            if (jsonrpcResponse.jsonrpc !== '2.0') {
                                console.error(
                                    `"jsonrpc" property is ${jsonrpcResponse.jsonrpc} not "2.0" as expected`,
                                    jsonrpcResponse
                                );
                                throw new Error(
                                    `"jsonrpc" property is ${jsonrpcResponse.jsonrpc} not "2.0" as expected`
                            ...
                            if (response.status !== 200) {
                                console.error(
                                    `Non-JSON body with Non-200 status ${response.status}: ${response.status}`,
                                    textResponse
                                );
                                throw new Error(
                                    `Non-JSON body with Non-200 status ${response.status}: ${response.status}`
