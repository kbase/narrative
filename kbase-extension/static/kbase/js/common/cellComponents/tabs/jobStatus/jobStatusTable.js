    ...
    function generateJobDisplayData(args) {
        ...
        Object.keys(typesToFiles).forEach((type) => {
            ...
            if (typesToFiles[type].appId === jobInfo.app_id) {
                ...
                Object.values(appData.specs[jobInfo.app_id].parameters).forEach((param) => {
                    const paramId = param.id;
                    // params generated by the job manager
                    if (jobInfo.userParams && jobInfo.userParams[paramId]) {
                        const value = jobInfo.userParams[paramId];
                        const label = param.ui_name;
                        params[paramId] = { value, label };
                        // params from a job info call
                    } else if (jobInfo.job_params && jobInfo.job_params[0][paramId]) {
                        const value = jobInfo.job_params[0][paramId];
                        const label = param.ui_name;
                        params[paramId] = { value, label };
                    }
                });
