define([], function () {
    ...
    function buildEditorRunner(cellId, runId, app, params) {
        var positionalArgs = [pythonifyValue(app.id), pythonifyValue(params, { autoIndent: true })],
            namedArgs = objectToNamedArgs({
                tag: app.tag,
                version: app.version,
                cell_id: cellId,
    ...
    function buildDataWidgetRunner(ref, cellId, title, tag) {
        var positionalArgs = [pythonifyValue(ref)];
    ...
    function buildCustomWidgetRunner(cellId, runId, app) {
        var positionalArgs = [pythonifyValue(app.id)],
            namedArgs = objectToNamedArgs({
                tag: app.tag,
                version: app.version,
                cell_id: cellId,
