define([
    ...
    'bootstrap',
    'css!font-awesome',
], function (Promise, html, Validation, Events, UI, lang, Resolver, FieldWidget) {
    'use strict';
    ...
    function factory(config) {
        ...
        function makeInputControl(events, bus) {
            ...
            return Promise.all(promiseOfFields).then(function (fields) {
                var layout = div(
                    {
                        class: 'row',
                    },
        ...
        function doChanged(id, newValue) {
            ...
            validate(viewModel.data).then(function (result) {
        ...
        function makeSingleInputControl(value, fieldSpec, events) {
            return resolver.loadInputControl(fieldSpec).then(function (widgetFactory) {
                var id = html.genId(),
        ...
        function renderSubcontrols() {
            if (viewModel.state.enabled) {
                ...
                return makeInputControl(events).then(function (result) {
        ...
        function autoValidate() {
            validate(viewModel.data).then(function (results) {
        ...
        function stop() {
            return Promise.try(function () {
                if (structFields) {
                    return Promise.all(
                        Object.keys(structFields).map(function (id) {
