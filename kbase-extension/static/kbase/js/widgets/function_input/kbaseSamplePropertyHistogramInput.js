    ...
    return KBWidget({
        ...
        fetchData: function (doneCallback) {
            ...
            if (self.state == self.STATE_NONE) {
                $(document).trigger('workspaceQuery.Narrative', function (ws_name) {
                        ...
                        function (result) {
                            ...
                            for (var i in self.rowMetadata) {
                                ...
                                for (var j in rm) {
                                    var propValue = rm[j];
                                    if (
                                        propValue.category == 'Sample' &&
                                        propValue.property_name == 'Name'
                                    ) {
                                        wellIds[propValue.property_value] =
                            ...
                            self.wellIdParams.sort(function (a, b) {
                                return a.text > b.text ? 1 : -1;
                            });

                            self.state = self.STATE_READY;
                            if (doneCallback) {
                                doneCallback(self.wellIdParams);
                            }
                        },
                        function (error) {
                            console.error(error);
