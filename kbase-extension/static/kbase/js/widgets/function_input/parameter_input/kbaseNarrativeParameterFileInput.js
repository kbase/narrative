...
], function (KBWidget, bootstrap, $, Config, kbaseNarrativeParameterInput, Shock) {
    'use strict';
    return KBWidget({
        ...
        render: function () {
            ...
            var tbl = $(
                '<table style="border: 0px; margin: 0px; cellpadding: 0px; cellspacing: 0px; width: 100%;"/>'
            );
            ...
            if (self.options.isInSidePanel) percentTextWidth = '100px';
            ...
            var $inputCol = $('<div>')
                ...
                .append(
                    $('<div>')
                        .css({ display: 'inline-block', height: '34px', 'vertical-align': 'top' })
                        .append($feedbackTip)
                );
            var $hintCol = $('<div>')
        ...
        fileSelected: function (nameText, prcText, realButton) {
            if (realButton.files.length != 1) return;
            ...
            var ujsKey = [
                'File',
                file.size,
                String(this.getFileLastModificationTime(file)),
                file.name,
                this.getUser(),
            ].join(':');
            ...
            ujsClient.get_has_state(
                this.options.serviceNameInUJS,
                ujsKey,
                0,
                function (data) {
                ...
                function (error) {
                    console.error(
                        'kbaseNarrativeParameterFileInput.fileSelected, in ujsClient.get_has_state: ',
                        error
                    );
            ...
            function processAfterNodeCheck(storedShockNodeId) {
                ...
                shockClient.upload_node(
                    file,
                    storedShockNodeId,
                    self.options.fullShockSearchToResume,
                    function (info) {
                        ...
                        if (info.uploaded_size) {
                            var shockNodeWasntDefined =
                                self.shockNodeId == null || self.shockNodeId !== info['node_id'];
                            if (shockNodeWasntDefined) {
                                self.shockNodeId = info['node_id'];
                                var fileState = self.shockNodeId + ' ' + curTime;
                                ujsClient.set_state(
                                    self.options.serviceNameInUJS,
                                    ujsKey,
                                    fileState,
                                    function (data) {
                            ...
                            if (info.uploaded_size >= info.file_size) {
                                ...
                                self.uploadWasStarted = false;
                                self.onChange();
                                shockClient.change_node_file_name(
                                    self.shockNodeId,
                                    file.name,
                                    function (info) {
                                    ...
                                    function (error) {
                                        console.error(
                                            'Error changing file name for shock node ' +
                                                self.shockNodeId
                                        );
                                        console.error(error);
                                    }
                    ...
                    function (error) {
                        console.error(
                            'kbaseNarrativeParameterFileInput.fileSelected, in shockClient.upload_node: ',
                            error
                        );
                        self.selectFileMode(true);
                        self.uploadWasStarted = false;
                    ...
                    function () {
                        console.log(
                            'kbaseNarrativeParameterFileInput.fileSelected, in shockClient.upload_node: check was-upload-cancelled=' +
                                self.cancelUpload
                        );
        ...
        getState: function () {
            return [
                [
                    this.shockNodeId ? this.shockNodeId : '',
                    this.fileName.val(),
                    this.uploadIsReady,
                    this.percentText.val(),
                ],
            ];
        },
        ...
        loadState: function (state) {
            if (!state) return;
            if (!$.isArray(state)) return;
            var self = this;
            this.shockNodeId = state[0][0];
            if (this.shockNodeId === '') this.shockNodeId = null;
        ...
        loggedInCallback: function (event, auth) {
            ...
            if (this.wrongToken) this.render();
            return this;
        },
