    ...
    return KBWidget({
        ...
        init: function(options) {
            ...
            const make_status_table = function(job_id, desc, status) {
                const status_box = $('<table class="table table-striped table-bordered" style="margin-left:auto; margin-right:auto">')
            ...
            const get_assemblies = function(job_id){
                const prom = $.get(self.arURL + '/user/' + user + '/job/' + job_id + '/results?type=contigs,scaffolds');
                return prom;
            };

            const get_best_assembly = function(job_id){
                const prom = $.get(self.arURL + '/user/' + user + '/job/' + job_id + '/assemblies/auto')
                return prom;
            };
            ...
            const data_report = $('<div class="panel panel-info" style="padding:10px">')
            ...
            const nav = $('<ul class="nav nav-pills"> \
            ...
            /////// Recipes
            const asm_div = $('<div class="row" id="frag-recipes"><small>Recipes are curated intelligent workflows designed for specific scenarios.</small></div>');
            const asm_choose = $('<span class="col-md-3"><select class="form-control" name="assemblers"> \
            ...
            const asm_desc = $('<span class="col-md-7"><input type="text" class="form-control" style="width:100%" placeholder="Description"></span>');
            const asm_btn = $('<span class="col-md-1 pull-right"><button class="btn btn-large btn-primary pull-right"><span class="glyphicon glyphicon-play-circle"  style="color:white;"></span> Assemble</button></span>');
            asm_div.append($('<fieldset><div class="form-group">').append(asm_choose, asm_desc, asm_btn));


            //////// Assemblers
            const asm_div2 = $('<div id="frag-assemblers"><small>Assemble contigs using multiple assemblers and compare results</small></div>');
            const asm_row = $('<div class="row">');
            const add_asm_group = $('<span class="btn-group col-md-2"></span>');
            const add_asm_btn = $('<button type="button" class="btn btn-warning asm-form dropdown-toggle" data-toggle="dropdown"> \
            ...
            const asm_desc2 = $('<div class="col-md-10"><input type="text" class="asm-form form-control" style="width:100%" placeholder="Description"></div>');
            const asm_btn2 = $('<button class="btn btn-default col-md-2 btn-primary pull-right asm-form"><span class="glyphicon glyphicon-play-circle"  style="color:white;"></span> Assemble</button>');

            var update_asm_pool = function(assembler_pool, pool){
                const body = $('<div>');
                for (i=0; i<pool.length;i++){
                    (function(i) {
                        body.append($('<div class="btn btn-primary btn-sm" style="margin:5px">' + pool[i] + '</div>').on('click', ()=> {
            ...
            get_assemblers().done((asms)=> {
                for (i=0; i<asms.length;i++){
                    (function(i) {
                        ...
                        const asm = $('<li data-toggle="tooltip" data-placement="right" title="' + asms[i].description + '" id="' + asm_id + '"><a>' + asms[i].name + '</a></li>');
            ...
            //////// Pipeline
            const asm_div3 = $('<div id="frag-pipeline"> <small>Build a customized pipeline using available preprocessing, assembly, and postprocessing tools</small></div>');
            const pipe_row1 = $('<div class="row">');
            const add_pipe_group = $('<span class="btn-group col-md-2"></span>');
            const add_pipe_btn = $('<button type="button" class="asm-form btn btn-large btn-warning dropdown-toggle" data-toggle="dropdown"> \
            ...
            const pipe_desc = $('<div class="col-md-10"><input type="text" class="asm-form form-control" style="width:100%" placeholder="Description"></div>');
            const pipe_btn = $('<button class="btn btn-large col-md-2 asm-form btn-primary pull-right"><span class="glyphicon glyphicon-play-circle"  style="color:white;"></span> Assemble</button>');

            var update_pipe_pool = function(pool){
                const body = $('<div>');
                for (i=0; i<pool.length;i++){
                    (function(i) {
                        body.append($('<div class="btn btn-primary btn-sm">' + pool[i] + '<span class="glyphicon glyphicon-chevron-right pull-right" style="color:white"></span></div>').on('click', ()=> {
                            console.log(pool[i]);
                    ...
                    (function(i) {
                        ...
                        for (j=0; j<asms.length;j++){
                            (function(j) {
                                ...
                                const asm2 = $('<li data-toggle="tooltip" data-placement="right" title="' + asms[j].description + '" id="' + asm_id + '"><a>' + asms[j].name + '</a></li>');
            ...
            const run_asm = function(arRequest) {
                ...
                $.ajax({
                    ...
                    success: function(data){
                        ...
                        const kill_btn = $('<span class="button btn btn-danger pull-right">Terminate</span>');
                        kill_btn.one("click", ()=> {
                            ...
                            $.when(prom).done((stat)=> {
                                ...
                                if (stat.search('Complete') != -1 || stat.search('FAIL') != -1) {
                                    ...
                                    if (stat.search('Complete') != -1) {
                                        ...
                                        $.when(defer_asm, defer_auto, defer_route, defer_report).done((assemblies, best, route, report)=> {
...
//                  import_contigs_to_ws(token, fba_url, ws_url, ws_name, shock_id, shock_url, cname)
        ...
        showResults: function(token, assemblies, best, route, report, job_id){
            ...
            if (!self.reloaded) {
                ...
                const import_btn = $('<button type="button" class="btn btn-default dropdown-toggle" data-toggle="dropdown"> Save Contigs <span class="caret"></span> </button>');
                ...
                const report_html = $('<iframe class="col-md-12" style="margin-top:15px" frameborder="0" height="1200px" src="' + full_link + '">');
                report_div += '<code style="font-size:4px>' + report +'</code><br>'
                const report_btn_toggle = $('<button class="btn btn-primary">Full Report</button>').on('click', ()=> {
                    report_html.toggle()
                })
