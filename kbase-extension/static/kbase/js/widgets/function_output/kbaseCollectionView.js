...
define([
    ...
    'kbStandaloneTable',
], function (KBWidget, bootstrap, $, Config, kbaseAuthenticatedWidget, kbStandaloneTable) {
    return KBWidget({
        name: 'CollectionView',
                    ...
                    if (data.length == 0) {
                        ...
                        if (idList.length > 0) {
                            kbws.get_objects(idList, function (resData) {
                                ...
                                var tlen = 0;
                                if (
                                    window.hasOwnProperty('rendererTable') &&
                                    rendererTable.length
                                ) {
                                    tlen = rendererTable.length;
                                }

                                var html =
                                    '<h4>Metagenome Collection ' +
                                    d.name +
                                    '</h4><div id="collectionTable' +
                                    tlen +
                                    '" style="width: 95%;"></div>';
                                container.empty();
                                ...
                                tableCollection.settings.target = document.getElementById(
                                    'collectionTable' + tlen
                                );
                                tableCollection.settings.data = {
                                    header: [
                                        'ID',
                                        'Name',
                                        'Project',
