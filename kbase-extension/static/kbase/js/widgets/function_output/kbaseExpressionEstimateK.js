...
define([
    ...
    'kbaseLinechart',
], function (KBWidget, bootstrap, $, Config, kbaseAuthenticatedWidget, kbaseLinechart) {
    return KBWidget({
        ...
        loadAndRender: function () {
            var self = this;
            self.loading(true);
            var ref = self.buildObjectIdentity(
                this.options.workspaceID,
                this.options.estimateKResultID
            );
        ...
        render: function () {
            ...
            $('<div>')
                .html('Estimated K (based on highest quality score) = ' + data.best_k)
                .appendTo(this.$elem);
            ...
            self.$avgWindowRefreshBtn.click(function () {
                ...
                if (!avgWindow) avgWindow = null;
                if (avgWindow < 0) avgWindow = null;
                if (avgWindow === null) self.$avgWindowTextField.val('');
            ...
            if (self.options.avgWindow) self.$avgWindowTextField.val(self.options.avgWindow);
            ...
            this.$elem.append(
                $('<a>')
                    .append('[Show Help on Silhouette Width]')
                    .on('click', function () {
                        $(this).hide();
                        $help.show();
                    })
