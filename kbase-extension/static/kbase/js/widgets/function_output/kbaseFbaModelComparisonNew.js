    ...
    return KBWidget({
        ...
        render: function () {
            ...
            container.append(
                '<div><img src="' +
                    self.loadingImage +
                    '">&nbsp;&nbsp;Loading model comparison data...</div>'
            );
                ...
                .then(
                    function (data) {
                        ...
                        for (var i = 0; i < modelComp.models.length; i++) {
                            tableStats.append(
                                '<tr><td><b>' +
                                    modelComp.models[i].id +
                                    '</b> genome</td><td>' +
                                    modelComp.models[i].name +
                                    '</td></tr>'
                            );
                        }
                        if (modelComp.pangenome_ref) {
                            tableStats.append(
                                '<tr><td>Pangenome</td><td>' +
                                    modelComp.pangenome_ref +
                                    '</td></tr>'
                            );
                        }
                        if (modelComp.protcomp_ref) {
                            tableStats.append(
                                '<tr><td>Proteome Comparison</td><td>' +
                                    modelComp.protcomp_ref +
                                    '</td></tr>'
                            );
                        }
                        tableStats.append(
                            '<tr><td>Conserved reactions</td><td>' +
                                modelComp.core_reactions +
                                '</td></tr>'
                        );
                        tableStats.append(
                            '<tr><td>Conserved compounds</td><td>' +
                                modelComp.core_compounds +
                                '</td></tr>'
                        );
                        tableStats.append(
                            '<tr><td>Conserved biomass compounds</td><td>' +
                                modelComp.core_biomass_compounds +
                                '</td></tr>'
                        );
                        tableStats.append(
                            '<tr><td>Conserved protein families</td><td>' +
                                modelComp.core_families +
                                '</td></tr>'
                        );
                        //////////////////////////////////////////// Reactions tab /////////////////////////////////////////////
                        ...
                        for (var i = 0; i < modelComp.reactions.length; i++) {
                            for (var j = 0; j < modelComp.models.length; j++) {
                                ...
                                if (
                                    modelComp.reactions[i].reaction_model_data.hasOwnProperty(
                                        model_id
                                    )
                                ) {
                                    if (
                                        modelComp.reactions[i].reaction_model_data[model_id][3] !==
                                        ''
                                    ) {
                                        modelComp.reactions[i]['model' + j] =
                                            modelComp.reactions[i].reaction_model_data[model_id][3];
                                    } else if (
                                        modelComp.reactions[i].reaction_model_data[model_id][0] ===
                                        1
                                    ) {
                        ...
                        for (var i = 0; i < modelComp.compounds.length; i++) {
                            for (var j = 0; j < modelComp.models.length; j++) {
                                ...
                                if (
                                    modelComp.compounds[
                                        i
                                    ].model_compound_compartments.hasOwnProperty(model_id)
                                ) {
                                    if (
                                        modelComp.compounds[i].model_compound_compartments[model_id]
                                            .length > 0
                                    ) {
                        ...
                        for (var i = 0; i < modelComp.biomasscpds.length; i++) {
                            for (var j = 0; j < modelComp.models.length; j++) {
                                var model_id = modelComp.models[j].id;
                                if (
                                    modelComp.biomasscpds[i].model_biomass_compounds.hasOwnProperty(
                                        model_id
                                    )
                                ) {
                                    if (
                                        modelComp.biomasscpds[i].model_biomass_compounds[model_id]
                                            .length > 0
                                    ) {
                        ...
                        for (var i = 0; i < modelComp.families.length; i++) {
                            for (var j = 0; j < modelComp.models.length; j++) {
                                var model_id = modelComp.models[j].id;
                                if (
                                    modelComp.families[i].family_model_data.hasOwnProperty(model_id)
                                ) {
                                    if (
                                        modelComp.families[i].family_model_data[model_id][0] === 1
                                    ) {
