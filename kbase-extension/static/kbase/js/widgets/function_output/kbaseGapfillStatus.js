    ...
    return KBWidget({
        ...
        render: function (options) {
            ...
            if (job_data.id) {
                ...
                panel.append(table);
                table.append(
                    '<tr><td>Gapfill Job was created with id:</td><td>' + job_data.id + '</td></tr>'
                );
                table.append(
                    '<tr><td>Gapfilling will run on model named</td><td>' +
                        job_data.jobdata.model +
                        '</td></tr>'
                );

                table.append(
                    '<tr><td>Based on your input parameters, the estimated runtime is:</td><td>' +
                        options.estimated_time_str +
                        '</td></tr>'
                );
                table.append(
                    '<tr><td>Current job state is</td><td id="' +
                        pref +
                        'job">' +
                        job_data.status +
                        '</td></tr>'
                ...
                var timeLst = function (event) {
                    kbws.get_jobs(
                        { auth: self.authToken(), jobids: [job_data.id] },
                        function (data) {
                            if (data.length == 0) {
                                var tdElem = $('#' + pref + 'job');
                                tdElem.html(
                                    '<span class="label label-danger">Error retrieving job status!</span>'
                                );
                                clearInterval(timer);
                            } else {
                                ...
                                if (status === 'done') {
                                    ...
                                    tdElem.html(
                                        '<span class="label label-success">Done! </span> &nbsp&nbsp View model details for gapfill solutions'
                                    );
                                    //ready();
                                } else {
                                    var tdElem = self.$elem.find('#' + pref + 'job');
                                    ...
                                    if (status === 'error') {
                                        console.error(
                                            'Error in kbaseGapfillStatus widget: ' +
                                        ...
                                        new kbaseNarrativeError(self.$elem, {
                                            error: {
                                                msg: errormssg + '\n\n' + mssg,
                        ...
                        function (data) {
                            var tdElem = self.$elem.find('#' + pref + 'job');
                            tdElem.html(
                                '<span class="label label-danger">Error connecting to jobs server!</span>'
                            );
                            console.error(
                                'Error in kbaseGapfillStatus widget: ' +
                                    JSON.stringify(data, null, 4)
