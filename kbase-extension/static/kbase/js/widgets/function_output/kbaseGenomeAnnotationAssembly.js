    ...
    return KBWidget({
        ...
        init: function init(options) {
            ...
            if (options._obj_info) {
                ...
                self.obj_ref =
                    self.assembly_info['ws_id'] +
                    '/' +
                    self.assembly_info['id'] +
                    '/' +
                    self.assembly_info['version'];
                self.link_ref =
                    self.assembly_info['ws_id'] +
                    '/' +
                    self.assembly_info['name'] +
                    '/' +
                    self.assembly_info['version'];
            }

            self.client = new GenericClient(Config.url('service_wizard'), {
            ...
            basicInfoCalls.push(
                Promise.resolve(
                    self.client.sync_call('AssemblyAPI.get_stats', [this.obj_ref])
                ).then((stats) => {
            ...
            for (const id in self.contig_lengths) {
                if (self.contig_lengths.hasOwnProperty(id)) {
                    ...
                    const contig = {
                        ...
                        len:
                            '<!--' +
                            self.contig_lengths[id] +
                            '-->' +
                            String(self.numberWithCommas(self.contig_lengths[id])) +
                            ' bp',
                        gc: gc,
                    };
        ...
        renderBasicTable: function () {
            ...
            $overviewTable.append(
                get_table_row('Number of Contigs', self.assembly_stats['num_contigs'])
            );
            $overviewTable.append(
                get_table_row(
                    'Total GC Content',
                    String((self.assembly_stats['gc_content'] * 100).toFixed(2)) + '%'
                )
            );
            $overviewTable.append(
                get_table_row(
                    'Total Length',
                    String(self.numberWithCommas(self.assembly_stats['dna_size'])) + ' bp'
                )
            );
            ...
            new DynamicTable($content, {
                ...
                updateFunction: function (pageNum, query, sortColId, sortColDir) {
                    ...
                    return Promise.resolve(
                        self.client.sync_call('AssemblyAPI.search_contigs', [
                            {
                                ref: self.obj_ref,
                                query: query,
                                sort_by: sortBy,
