    ...
    return KBWidget({
        ...
        render: function () {
            ...
            if (data) {
                ...
                panel.append(table);
                table.append(
                    '<tr><td>Job was created with id</td><td>' + self.options.job_id + '</td></tr>'
                );
                table.append(
                    '<tr><td>Output model will have the id</td><td>' +
                        self.options.model_id +
                        '</td></tr>'
                );
                table.append(
                    '<tr><td>Current job state is</td><td id="' + pref + 'job"></td></tr>'
                );
                var timeLst = function (event) {
                    kbws.get_jobs(
                        { auth: self.authToken(), jobids: [self.options.job_id] },
                        function (data) {
                            ...
                            if (status === 'done') {
                                ...
                                kbws.get_object(
                                    {
                                        auth: self.authToken(),
                                        workspace: self.options.ws_name,
                                        id: self.options.model_id,
                                        type: 'Model',
                                    },
                                    function (ret) {
                                        $('.loader-table').remove();
