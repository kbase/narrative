    ...
    return KBWidget({
        ...
        init: function (options) {
            ...
            if ('modelsData' in options) {
                ...
                container.append(
                    '<div id="modeltabs-loading"><img src="' +
                        self.loadingImage +
                        '">&nbsp&nbsploading model...</div>'
                );

                const fba = new fbaModelServices(this.fbaURL, { auth: self.authToken() });
                fba.get_models(
                    { auth: self.authToken(), workspaces: [options.ws], models: [options.id] },
                    (data) => {
        ...
        render: function (options) {
            ...
            container.append(
                '<i>' +
                    ...
                    ' reactions, ' +
                    model.compounds.length +
                    ' compounds, ' +
                    parseInt(
                        model.integrated_gapfillings.length + model.unintegrated_gapfillings.length
                    ) +
                    ' gapfill runs</i><br>'
            );
            ...
            var keys = ['id', 'name', 'definition', 'features', 'id'];
            var labels = [
                'Id (compartment)',
                'Name',
                'Equation',
            ...
            function gapFillTable(models) {
                        ...
                        if (intGap.length == 6) {
                            ...
                            intGap.splice(
                                ...
                                intGap[1] +
                                    ...
                                    intGap[2] +
                                    ')&nbsp&nbsp' +
                                    '<a class="show-gap" data-ref="' +
                                    intGap[1] +
                                    '" >view solution details</a>'
                            );
                        }
                    }
                ...
                function showGapfillSolutions(tr, gapRef) {
                    ...
                    const gapAJAX = fba.get_gapfills({
                        gapfills: [gapRef],
                        workspaces: [self.currentws],
                        auth: self.authToken(),
                    });
                    $.when(gapAJAX).done((data) => {
                        ...
                        const solList = $('<div>').append(
                            '<br> ' + sols.length + ' solutions found by this gapfill run<br>'
                        );
                        ...
                        for (const i in sols) {
                            ...
                            solList.append(
                                '<br><i>solution id:</i>&nbsp&nbsp<b>' + solID + '<b><br>'
                            );
                            ...
                            if (rxnAdditions.length == 0) {
                                ...
                                const createTableRow = function (rxn_id, equation) {
                                    return (
                                        '<tr><td>' + rxn_id + '</td><td>' + equation + '</td></tr>'
                                    );
                                };
