    ...
    return KBWidget({
        ...
        render: function () {
            ...
            container.append(
                '<div><img src="' +
                    self.loadingImage +
                    '">&nbsp;&nbsp;loading phenotype simulation data...</div>'
            );
            ...
            kbws.get_objects(
                [{ ref: self.ws_name + '/' + self.simulation_id }],
                function (data) {
                    container.empty();
                    ...
                    table.append(
                        '<tr><th' +
                            ...
                            '><b>Media</b></th><th' +
                            s +
                            '><b>Original growth</b></th>\
        				<th' +
                            s +
                            '><b>Simulated growth</b></th><th' +
                            s +
                            '><b>Simulation class</b></th></tr>'
                    );
                    ...
                    for (var pos in simList) {
                        ...
                        var pheno = sim[0];
                        var geneKO = '' + pheno[1];
                        if (geneKO.length == 0) geneKO = '-';
                        var media = '' + pheno[1];
                        var addCmp = '' + pheno[3];
                        if (addCmp.length > 0) media = media + '<br>(' + pheno[3] + ')';
                        var original = '' + pheno[4];
                        var simulated = '' + sim[1];
                        var simClass = '' + sim[3];
                        var color = '#00AA00';
                        if (original != simulated) color = '#AA0000';
