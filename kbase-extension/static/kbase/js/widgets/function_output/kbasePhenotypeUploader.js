(function ($, undefined) {
    return KBWidget({
        ...
        init: function (options) {
            ...
            container.append(
                '<div><img src="' + self.loadingImage + '">&nbsp;&nbsp;loading media data...</div>'
            );
            ...
            kbws.list_objects(
                ...
                function (data) {
                    ...
                    table.append(
                        '<tr><td>Storing phenotype set ID</td><td>' +
                            options.phenotype_id +
                            '</td></tr>'
                    );
                    table.append(
                        '<tr><td>Genome object ID</td><td>' + options.genome_id + '</td></tr>'
                    );
                    table.append(
                        '<tr><td>Your tab delimited data<br>(first line is header)</td> \
            			<td><textarea id="' +
                            pref +
                            'pheno-ta" style="width:100%;" cols="80" rows="15"></textarea></td></tr>'
                    );
                    container.append(
                        '<button class="btn" id="' + pref + 'pheno-btn">Save in workspace</button>'
                    );
                    container.append(
                        '&nbsp;<button class="btn" id="' +
                            pref +
                    ...
                    $('#' + pref + 'pheno-btn').click(function () {
                        ...
                        for (var pos in lines) {
                            ...
                            if (parts.length != 5) {
                                good = false;
                                alert(
                                    'Data line has wrong format: [' +
                                        line +
                                        ']. It should be in format: media_id{TAB}workspace{TAB}growth(0_or_1){TAB}geneKO(or_none){TAB}addtlCpd(or_none)'
                                );
                                break;
                            }
                            if ($.inArray(parts[0], medias) < 0) {
                                good = false;
                                alert(
                                    'Data line contains unknown media: [' +
                                        line +
                                        ']. Media could be one of: ' +
                                        medias
                                );
                                break;
                            }
                            if (parts[1] != options.ws_name) {
                                good = false;
                                alert(
                                    'Data line contains different workspace: [' +
                                        line +
                                        ']. It should refer to current workspace: ' +
                                        options.ws_name
                                );
                                break;
                            }
                            if (parts[2] != '0' && parts[2] != '1') {
                                good = false;
                                alert(
                                    'Data line contains wrong growth value: [' +
                                        line +
                                        ']. It should be 0 or 1'
                                );
                                break;
                    ...
                    $('#' + pref + 'xmpl-btn').click(function () {
                        ...
                        var text =
                            'media	mediaws	growth	geneko	addtlCpd\n' +
                            'C-L-serine	' +
                            ws +
                            '	1	none	cpd00367,cpd00009,cpd00048\n' +
                            'C-L-serine	' +
                            ws +
                            '	1	none	cpd00023,cpd00009,cpd00048\n' +
                            'C-L-serine	' +
                            ws +
                            '	1	none	cpd00221,cpd00159,cpd00013,cpd00048,cpd00046\n' +
                            'C-L-serine	' +
                            ws +
                            '	1	none	cpd00221,cpd00159,cpd00013,cpd00048,cpd00080\n' +
                            'C-L-serine	' +
                            ws +
                            '	0	none	cpd00122,cpd00013,cpd00009,cpd00048\n' +
                            'C-L-serine	' +
                            ws +
                            '	0	none	cpd00013,cpd00009,cpd00048,cpd00141\n' +
                            'C-L-serine	' +
                            ws +
                            '	0	none	cpd00122,cpd00009,cpd00048\n' +
                            'C-L-serine	' +
                            ws +
                            '	0	none	cpd00161,cpd00009,cpd00048\n';
                        var val = $('#' + pref + 'pheno-ta').val();
                        if (val != '') val += '\n';
                        val += text;
