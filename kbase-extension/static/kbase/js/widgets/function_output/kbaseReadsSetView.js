define([
    ...
    return KBWidget({
        ...
        init: function (options) {
            ...
            $self.setAPI = new SetAPI(Config.url('service_wizard'), { token: this.authToken() });

            return Promise.resolve(
                $self.setAPI.get_reads_set_v1({
                    ref: $self.obj_ref,
                    include_item_info: 1,
                })
            )
        ...
        update_overview_info_from_nar_info: function (info) {
            ...
            $self.set_overview.link_ref =
                info['ws_id'] + '/' + info['name'] + '/' + info['version'];
        ...
        render: function () {
            ...
            $overviewTable.append(
                '<tr><th><b>Description</b></th><td>' +
                    $self.set_overview.description +
                    '</td></tr>'
            );
            $overviewTable.append(
                '<tr><th><b>Read Libraries</b></th><td>' +
            ...
            $divs.append(
                $('<div id="' + tab_ids.overview + '" class="tab-pane active">').append(
                    $('<div>').css('margin-top', '15px').append($overviewTable)
                )
            );
            ...
            $divs.append(
                $('<div id="' + tab_ids.browse + '" class="tab-pane">').append(
                    $('<div>')
