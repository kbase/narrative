    ...
    return KBWidget({
            ...
            if (this.options.obj_ref) {
                ...
                if (this.options._obj_info) {
                    this.reference =
                        ...
                        this.options.objId +
                        '/' +
                        this.options._obj_info['version'];
                } else {
                    this.reference = this.options.wsName + '/' + this.options.objId;
                }
            }
            Promise.resolve(
                this.client.sync_call('ReadsAPI.get_reads_info_all_formatted', [
                    { workspace_obj_ref: this.reference },
                ])
            )
                .then(
                    function (results) {
        ...
        renderBasicTable: function () {
            var $container = this.$elem,
                ...
                $overviewTable = $(
                    '<table class="table table-striped table-bordered table-hover" style="margin-left: auto; margin-right: auto; word-wrap: break-word; table-layout: fixed;"/>'
            ...
            */
            ...
            if (this.reads['Type'] === 'Paired End') {
                $overviewTable.append(
                    get_table_row('Insert Size Mean', this.reads['Insert_Size_Mean'])
                );
                $overviewTable.append(
                    get_table_row('Insert Size Std Dev', this.reads['Insert_Size_Std_Dev'])
                );
                $overviewTable.append(
                    get_table_row(
                        'Outward Read Orientation',
                        this.reads['Outward_Read_Orientation']
                    )
                );
            }
            $divs.append(
                $('<div id="' + tab_ids.overview + '" class="tab-pane active">').append(
                    $overviewTable
                )
            );
            ...
            //Stats portion
            $statsTable.append(
                get_table_row('Total Number of Bases', this.reads['Total_Number_of_Bases'])
            );
            $statsTable.append(get_table_row('Mean Read Length', this.reads['Mean_Read_Length']));
            $statsTable.append(
                get_table_row('Read Length Std Dev', this.reads['Read_Length_Std_Dev'])
            );
            $statsTable.append(
                get_table_row(
                    'Number of Duplicate Reads(%)',
                    this.reads['Number_of_Duplicate_Reads']
                )
            );
            $statsTable.append(get_table_row('Phred Type', this.reads['Phred_Type']));
            $statsTable.append(
                get_table_row(
                    'Quality Score Mean (Std Dev)',
                    this.reads['Quality_Score_Mean_Std_Dev']
                )
            );
            $statsTable.append(
                get_table_row('Quality Score (Min/Max)', this.reads['Quality_Score_Min_Max'])
            );
            ...
            $divs.append(
                $('<div id="' + tab_ids.stats + '" class="tab-pane">').append($statsTable)
            );
