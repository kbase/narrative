...
define(['kbwidget', 'bootstrap', 'jquery', 'narrativeConfig', 'd3'], (
    KBWidget,
    bootstrap,
    $,
    Config,
    ...
    return KBWidget({
        ...
        */
        loadSEEDHierarchy: function () {
            ...
            d3.text('/modules/plugins/dataview/resources/data/subsys.txt', (text) => {
                    ...
                    for (j = 0; j < ontologyDepth; j++) {
                        ...
                        if (j === 0) {
                            ...
                            if (nodeMap[nodeHierarchy] === undefined) {
                                ...
                                if (
                                    j === ontologyDepth - 1 &&
                                    subsysToGeneMap[data[i][j]] !== undefined
                                ) {
                                    subsysToGeneMap[data[i][j]].forEach((f) => {
                                        const gene = { name: f, size: '' };
                ...
                if (totalGenesWithFunctionalRoles < 100) {
                    ...
                    self.$elem
                        .find('#mainview')
                        .append(
                            'No Functional Categories assigned, you can added them using the Narrative'
                        );
                } else {
        ...
        update: function (source) {
            ...
            const scale = d3.scale.linear().domain([0, this.maxCount]).range([0, 275]);
        ...
        render: function () {
            ...
            const $mainview = $('<div id="mainview">').css({ 'overflow-x': 'scroll' });
            ...
            const obj = { ref: this.options.wsNameOrId + '/' + this.options.objNameOrId };
            let prom;
