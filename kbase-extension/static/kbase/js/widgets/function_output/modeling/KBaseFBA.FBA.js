    ...
    function KBaseFBA_FBA(modeltabs) {
        ...
        this.tabList = [
            ...
            },
            {
                ...
                getParams: function () {
                    return { models: [self.model], fbas: [self.data] };
                },
            },
        ];
        ...
        this.formatObject = function () {
            ...
            for (var key in self.data.compoundflux_objterms) {
                this.usermeta.objectivefunction +=
                    ' (' + self.data.compoundflux_objterms[key] + ') ' + key;
            }
            for (var key in self.data.reactionflux_objterms) {
                this.usermeta.objectivefunction +=
                    ' (' + self.data.reactionflux_objterms[key] + ') ' + key;
            }
            for (var key in self.data.biomassflux_objterms) {
                this.usermeta.objectivefunction +=
                    ' (' + self.data.biomassflux_objterms[key] + ') ' + key;
            }
            this.usermeta.objectivefunction += '}';
        ...
        this.setData = function (indata, tabs) {
            ...
            var p = self.modeltabs
                .kbapi('ws', 'get_objects', [{ ref: indata.fbamodel_ref }])
                .then(function (data) {
                    ...
                    if (tabs != undefined) {
                        self.modeltabs
                            .kbapi('ws', 'get_objects', [{ ref: self.model.template_ref }])
                            .then(function (data) {
                                var $usePlantModel = 0;
