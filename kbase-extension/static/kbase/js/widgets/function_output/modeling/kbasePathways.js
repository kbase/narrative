define(['kbwidget', 'jquery', 'bootstrap', 'kbaseTabTableTabs'], function (
    KBWidget,
    $,
    bootstrap,
    kbaseTabTableTabs
    ...
    return KBWidget({
        ...
        init: function (options) {
            ...
            var tabs = new kbaseTabTableTabs(container, {
            ...
            function events() {
                ...
                container.find('.pathway-link').click(function () {
                    ...
                    var container = $(
                        '<div id="path-container-' + elemID + '" style="position:relative;">'
                    );
                    container.loading();
                ...
                container
                    .find('.pathway-link')
                    .tooltip({ title: 'Open path tab', placement: 'right', delay: { show: 1000 } });
            } // end events

            function load_map(map, container, elemID) {
                var p1 = kbapi('ws', 'get_objects', [
                        { workspace: imageWorkspace, name: map + '.png' },
                    ]),
                    p2 = kbapi('ws', 'get_objects', [{ workspace: mapWorkspace, name: map }]);
                $.when(p1, p2).then(function (imgRes, mapRes) {
                    ...
                    container.append(
                        '<div id="pathway-' + elemID + '" style="position:absolute; top:0;">'
                    );

                    container.rmLoading();
                    new ModelSeedPathway({
                        elem: 'pathway-' + elemID,
                        usingImage: true,
                        mapData: mapData,
                        models: models,
