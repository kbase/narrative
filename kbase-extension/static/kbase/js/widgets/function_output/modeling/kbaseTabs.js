...
define(['kbwidget', 'jquery', 'bootstrap'], function (KBWidget, $, bootstrap) {
    return KBWidget({
        ...
        init: function (options) {
            ...
            this.addTab = function (p) {
                ...
                if (tabs.find('a[data-id="' + p.name + '"]').length > 0) return;

                var tab = $('<li class="' + (p.active ? 'active' : '') + '">');
                var tab_link = $(
                    '<a data-toggle="tab" data-id="' + p.name + '">' + p.name + '</a>'
                );
            ...
            this.rmTab = function (name) {
                ...
                if (tab.next().length > 0) var id = tab.next().children('a').data('id');
                else var id = tab.prev().children('a').data('id');
