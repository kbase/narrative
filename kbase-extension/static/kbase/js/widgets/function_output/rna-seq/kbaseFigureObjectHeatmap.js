define(['kbwidget', 'bootstrap', 'jquery', 'kbaseAuthenticatedWidget', 'kbaseHeatmap'], function (
    KBWidget,
    bootstrap,
    $,
    ...
    return KBWidget({
        ...
        setDataset: function setDataset(newDataset, groupInfo) {
            ...
            if (newDataset.data.length == 0) {
                ...
                var needs_decrement = Object.keys(rows_by_cluster).reduce(function (
                    needs_it,
                    label
                ) {
                    if (label === 'Cluster_0') {
                ...
                var ygtick_labels = Object.keys(rows_by_cluster)
                    .sort()
                    .map(function (label) {
                        ...
                        if (needs_decrement) {
                            label = label.replace(/(\d+)$/, function (m, d) {
                                return d - 1;
                            });
                        }
                        return label;
                ...
                var ygroup = Object.keys(rows_by_cluster)
                    .sort()
                    .map(function (label) {
