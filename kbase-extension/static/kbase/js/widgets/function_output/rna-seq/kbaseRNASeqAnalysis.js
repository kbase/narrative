define(['kbwidget', 'bootstrap', 'jquery', 'kbaseTable', 'kbaseAuthenticatedWidget'], (
    KBWidget,
    bootstrap,
    $,
    kbaseTable,
    ...
    return KBWidget({
        ...
        loadAnalysis: function (ws, analysis) {
            ...
            if (analysis.sample_ids) {
                $.each(analysis.sample_ids, (i, v) => {
                    num_sample_ids++;
                    all_promises.push(ws.get_object_info([{ ref: v }]));
                });
            }
            ...
            if (analysis.alignments) {
                ...
                const alignment_ids = [];
                $.each(analysis.alignments, (k, v) => {
                    if (external_ids[k] == undefined) {
                        alignment_ids.push(k);
                        all_promises.push(ws.get_object_info([{ ref: k }]));
                        external_ids[k] = 1;
                    }
            ...
            if (analysis.expression_values) {
                ...
                const ev_ids = [];
                $.each(analysis.expression_values, (k, v) => {
                    if (external_ids[k] == undefined) {
                        ev_ids.push(k);
                        all_promises.push(ws.get_object_info([{ ref: k }]));
                        external_ids[k] = 1;
            ...
            if (analysis.transcriptome_id) {
                ...
                all_promises.push(ws.get_object_info([{ ref: analysis.transcriptome_id }]));
            }

            if (analysis.cuffdiff_diff_exp_id) {
                this.options.tableColumns.push('Cuffdiff Output');
                all_promises.push(ws.get_object_info([{ ref: analysis.cuffdiff_diff_exp_id }]));
            }
            ...
            $.when
                ...
                .then(function (annotation, genome) {
                    ...
                    if (args.length > 2) {
                        ...
                        if (analysis.sample_ids) {
                            $rna.dataset().parsed_read_samples = $rna.ulFromData(
                                analysis.sample_ids,
                                ref_map
                            );
                        }
                        ...
                        if (analysis.alignments) {
                            $rna.dataset().parsed_alignments = $rna.ulFromData(
                                analysis.alignments,
                                ref_map
                            );
                        }
                        ...
                        if (analysis.expression_values) {
                            $rna.dataset().parsed_expression_values = $rna.ulFromData(
                        ...
                        if (analysis.transcriptome_id) {
                            $rna.dataset().parsed_transcriptome = $rna.ulFromData(
                                [analysis.transcriptome_id],
                                ref_map
                            );
                        }

                        if (analysis.cuffdiff_diff_exp_id) {
                            $rna.dataset().parsed_cuffdiff = $rna.ulFromData(
                                [analysis.cuffdiff_diff_exp_id],
                                ref_map
                            );
                        }
                    }
                        ...
                        $.jqElem('li').append(
                            $.jqElem('a')
                                ...
                                .on('click', (e) => {
                                    ...
                                    const near_idx = IPython.notebook.find_cell_index(
                                        $cell.data().cell
                                    );
