...
(function () {
    ...
    var standaloneGraph = (root.standaloneGraph = {
        ...
        hover: function (title, value, event, e) {
            ...
            if (title) {
                jQuery(this, svg.root()).attr('fill-opacity', 0.8);
            ...
            if (event == 'click') {
                var num = parseInt(
                    this.parentElement.className.baseVal.substr(
                        this.parentElement.className.baseVal.search(/\d+/)
                    )
                );
        ...
        drawImage: function (svg, cmax, index) {
            ...
            svg.linearGradient(defs, 'fadeRed', [
                [0, '#EE5F5B'],
                [1, '#BD362F'],
            ]);
            ...
            svg.linearGradient(defs, 'fadeYellow', [
                [0, '#FBB450'],
                [1, '#F89406'],
            ]);
            svg.linearGradient(defs, 'fadeLightblue', [
                [0, '#5BC0DE'],
                [1, '#2F96B4'],
            ]);
            svg.linearGradient(defs, 'fadePurple', [
                [0, '#ee5be0'],
                [1, '#bd2fa6'],
            ]);
            ...
            svg.graph.xAxis
                .title(renderer.settings.x_title, renderer.settings.x_title_color)
            ...
            var sy = rendererGraph[0].niceScale({
                min: 0,
                max: max,
                ticks: renderer.settings.y_labeled_tick_interval,
            });
            svg.graph.yAxis
                .title(renderer.settings.y_title, renderer.settings.y_title_color)
                .ticks(
                    sy.max / renderer.settings.y_labeled_tick_interval,
                    sy.max / renderer.settings.y_tick_interval,
                    null,
                    null,
            ...
            if (renderer.settings.hasY2) {
                svg.graph.y2Axis
                    ...
                    .ticks(
                        parseInt(y2max / renderer.settings.y2_labeled_tick_interval),
                        parseInt(y2max / renderer.settings.y2_tick_interval),
                        null,
                        null,
                        renderer.settings.y_scale
                    )
            ...
            svg.graph
                .noDraw()
                .legend.show(renderer.settings.show_legend)
                .area(
            ...
            for (i = 0; i < renderer.settings.data.length; i++) {
                svg.graph
                    .noDraw()
                    .series(i)
                    .format(renderer.settings.data[i].fill || fills[i])
                    .end();
            }
            svg.graph
                .noDraw()
            ...
            for (var i = 0; i < data.length; i++) {
                ...
                if (data[i].data.length % 2 == 1) {
                    ...
                    if ((med + 1) % 2 == 1) {
                        ...
                        fivenumbers[i]['lower'] =
                            (data[i].data[(med + 1) / 2] + data[i].data[(med + 1) / 2 + 1]) / 2;
                        fivenumbers[i]['upper'] =
                            (data[i].data[med + (med + 1) / 2 - 1] +
                                data[i].data[med + (med + 1) / 2]) /
                            2;
                    }
                } else {
                    ...
                    if (medup % 2 == 1) {
                        ...
                        fivenumbers[i]['lower'] =
                            (data[i].data[medup / 2 - 1] + data[i].data[medup / 2]) / 2;
                        fivenumbers[i]['upper'] =
                            (data[i].data[medup + medup / 2 - 1] +
                                data[i].data[medup + medup / 2]) /
                            2;
                    }
                }
            }
