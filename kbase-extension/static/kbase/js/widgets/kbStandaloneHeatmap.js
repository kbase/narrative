...
(function () {
    ...
    const standaloneHeatmap = (root.standaloneHeatmap = {
        ...
        render: function (index) {
            ...
            const min_height =
                renderer.settings.data.rows.length * renderer.settings.min_cell_height +
                renderer.settings.tree_height +
                renderer.settings.legend_height;
            if (renderer.settings.height < min_height) {
                renderer.settings.height = min_height;
            }
            const min_width =
                renderer.settings.data.columns.length * renderer.settings.min_cell_height +
                renderer.settings.tree_width +
                renderer.settings.legend_width;
            ...
            rendererHeatmap[index].drawImage(
                jQuery('#heatmap_div' + index).svg('get'),
                renderer.index
            );
        ...
        drawImage: function (svg, index) {
            ...
            if (
                renderer.settings.data.hasOwnProperty('coldend') &&
                renderer.settings.data.hasOwnProperty('colindex')
            ) {
                renderer.settings.data.colcluster = renderer.settings.data.coldend;
            } else {
                const col_result = renderer.cluster(
                    renderer.transpose(renderer.settings.data.data)
                );
                renderer.settings.data.colcluster = col_result[0];
                renderer.settings.data.colindex = col_result[1];
            }
            if (
                renderer.settings.data.hasOwnProperty('rowdend') &&
                renderer.settings.data.hasOwnProperty('rowindex')
            ) {
            ...
            for (let i = 0; i < renderer.settings.data.data.length; i++) {
                ...
                const texty =
                    ...
                    renderer.settings.legend_height +
                    (boxheight * (i + 1) -
                        parseInt((boxheight - renderer.settings.row_text_size) / 2)) -
                    2;
                let fontColor = 'black';
                ...
                svg.text(
                    ...
                    textx,
                    texty,
                    '' + renderer.settings.data.rows[renderer.settings.data.rowindex[i] - 1],
                    {
                        fill: fontColor,
                        fontSize: renderer.settings.row_text_size + 'px',
                        onclick:
                            'rendererHeatmap[' +
                            index +
                            '].toggleSelected(' +
                ...
                for (let h = 0; h < renderer.settings.data.data[i].length; h++) {
                    ...
                    if (i == 0) {
                        const ctextx =
                            renderer.settings.tree_width +
                            boxwidth * h +
                            parseInt((boxwidth - renderer.settings.col_text_size) / 2) +
                            12;
                        ...
                        svg.text(
                            ...
                            ctextx,
                            ctexty,
                            renderer.settings.data.columns[renderer.settings.data.colindex[h] - 1],
                            {
                                fill: fontColor,
                                fontSize: renderer.settings.col_text_size + 'px',
                    ...
                    const adjusted_value =
                        renderer.settings.data.data[renderer.settings.data.rowindex[i] - 1][
                            renderer.settings.data.colindex[h] - 1
                        ] *
                            2 -
                        1;
                    ...
                    // draw the box
                    renderer.settings.cells[i][h] = svg.rect(
                        null,
                        x,
                        y,
                        width,
        ...
        drawDendogram: function (svg, index, rotation) {
            ...
            const data = rotation
                ? renderer.settings.data.rowcluster
                : renderer.settings.data.colcluster;
            ...
            if (rotation) {
                ...
                for (var i = 0; i < data.depth; i++) {
                    ...
                    for (var h = 0; h < data[i].length; h++) {
                        ...
                        curr_shift += cluster.b
                            ? (cluster.a + cluster.b) * cell_w
                            : cluster.a * cell_w;
                    }
                    xshift -= interval;
                }
        ...
        cluster: function (data) {
            ...
            while (num_avail > 1) {
                ...
                for (var i = 0; i < row_a.length; i++) {
                    row_a[i] =
                        row_a[i] / (clusters[coord_a].data.length + clusters[coord_b].data.length);
                }
            ...
            for (var i = data.length; i < clusters.length; i++) {
                ...
                if (clusters[i].level[0] != clusters[i].level[1]) {
                    ...
                    for (
                        var h = 0;
                        h < Math.abs(clusters[i].level[0] - clusters[i].level[1]);
                        h++
                    ) {
