...
function KBCacheClient(token) {
    ...
    this.getNarratives = function () {
        ...
        var last_prom = $.when(p).then(function (data) {
            ...
            var my_prom = kb.ws.list_objects({
                workspaces: mine_ws,
                type: 'KBaseNarrative.Narrative',
                showHidden: 1,
            });

            var shared_prom = kb.ws.list_objects({
                workspaces: shared_ws,
                type: 'KBaseNarrative.Narrative',
                showHidden: 1,
            });

            var public_prom = kb.ws.list_objects({
...
function UIUtils() {
    ...
    this.notify = function (text, type, keep) {
        ...
        $('#notification')
            .delay(200)
            .animate({ top: 50 }, 400, 'linear', function () {
                if (!keep) {
                    $('#notification')
                        .delay(2000)
    ...
    var dayOfWeek = { 0: 'Sun', 1: 'Mon', 2: 'Tues', 3: 'Wed', 4: 'Thurs', 5: 'Fri', 6: 'Sat' };
    var months = {
        0: 'Jan',
        1: 'Feb',
        2: 'March',
