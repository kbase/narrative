    ...
    return KBWidget({
        ...
        */
        ...
        getItemParents: function (item_info) {
            ...
            return _.map(_.keys(this.setItems[item_id]), function (key) {
                return {
                    item_id: key,
        ...
        init: function (options) {
            ...
            this.$addDataButton = $('<button>')
                ...
                .click(function () {
                    ...
                    _this.trigger(
                        'toggleSidePanelOverlay.Narrative',
                        _this.options.parentControlPanel.$overlayPanel
                    );
                });
            var $mainListDivContainer = $('<div>')
        ...
        refresh: function (showError) {
            ...
            if (this.refreshTimer === null) {
                ...
                ); // check if there is new data every X ms
            }
        ...
        refreshTimeStrings: function () {
            Object.keys(this.dataObjects).forEach(
                function (i) {
            ...
            this.fetchWorkspaceData()
                .then(
                    function () {
                        ...
                        var justInfo = Object.keys(this.dataObjects).map(
                            function (objId) {
                                return this.dataObjects[objId].info;
                            }.bind(this)
                        );
                        var objectInfoPlus = Object.keys(this.dataObjects).map(
                            function (objId) {
                            ...
                            this.viewOrder.sort(
                                function (a, b) {
                                    var idA = a.objId,
                                        idB = b.objId;
                                    if (
                                        this.dataObjects[idA].info[3] >
                                        this.dataObjects[idB].info[3]
                                    ) {
                                        return -1;
                                    }
                                    if (
        ...
        fetchWorkspaceData: function () {
            ...
            var updateSetInfo = function (obj) {
                ...
                obj.set_items.set_items_info.forEach(
                    function (setItem) {
        ...
        makeToolbarButton: function (name) {
            ...
            var $btn = $('<span>').addClass(btnClasses).css(btnCss);
                                            ...
                                            .append(
                                                $('<td>').append(
                                                    'Saved by ' +
                                                        history[k][5] +
                                                        '<br>' +
                                                        TimeFormat.getTimeStampStr(history[k][3])
                                                )
                                            )
                                            .append(
                                                $('<td>')
        ...
        addDataControls: function (objData, $alertContainer, ref_path) {
            ...
            var $btnToolbar = $('<span>').addClass('btn-group');
                    ...
                    function (result) {
                        ...
                        return workspace
                            ...
                            .spread(function (result) {
                                var objectReportsForOutput = result.data
                                    .filter(function (reportObject) {
                                        // pull out found objects which are in the list of objects created in the report.
                                        // objects_created looks like {description: .., ref: ..}
                                        return reportObject.data.objects_created.some(function (
                                            objectCreated
                                        ) {
        ...
        onOpenDataListItem: function ($moreRow, objData) {
            ...
            this.getReportForObject(objectInfo).then(function (result) {
                if (result.length === 0) {
        ...
        renderObjectRowDiv: function (objId) {
            ...
            var $moreContent = $('<div>')
                ...
                .append(_this.addDataControls(objData, $alertDiv, ref_path))
                .append($alertDiv)
                .append(
        ...
        renderController: function () {
            ...
            var $upOrDown = $('<button class="btn btn-default btn-sm" type="button">')
                .css({ 'margin-left': '5px' })
                .append(
            ...
            var $byDate = $('<label id="nar-data-list-default-sort-label" class="btn btn-default">')
                ...
                .on('click', function () {
                    _this.sortData(function (a, b) {
                        return (
                            _this.sortOrder *
                            _this.dataObjects[a.objId].info[3].localeCompare(
                                _this.dataObjects[b.objId].info[3]
                            )
                        );
                    });
                ...
                .on('click', function () {
                    _this.sortData(function (a, b) {
                        return (
                            -1 *
                            _this.sortOrder *
                            _this.dataObjects[a.objId].info[1]
                                .toUpperCase()
            ...
            var $byType = $('<label class="btn btn-default">')
                ...
                .on('click', function () {
                    _this.sortData(function (a, b) {
                        var aType = _this.dataObjects[a.objId].info[2]
                            .toUpperCase()
                            .match(/\.(.+)/)[1];
                        var bType = _this.dataObjects[b.objId].info[2]
                            .toUpperCase()
                            .match(/\.(.+)/)[1];
            ...
            _this.$filterTypeDiv = $('<div>').append(_this.$filterTypeSelect);
        ...
        populateAvailableTypes: function () {
            if (this.availableTypes && this.$filterTypeSelect) {
                        ...
                        function (type) {
                            ...
                            this.$filterTypeSelect.append(
                                $('<option value="' + typeInfo.type + '">').append(
                                    [typeInfo.type, ' (', typeInfo.count, ' object', suf, ')'].join(
                                        ''
                                    )
                                )
                            );
                ...
                this.$filterTypeSelect
                    .prepend(
                        $('<option value="">').append(
                            'Show All Types (' + runningCount + ' object' + suf + ')'
                        )
                    )
                    .val(selected);
            }
