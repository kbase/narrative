...
define([
    ...
    'base/js/namespace',
], function (Promise, KBWidget, $, Config, kbase_client_api, GenericClient, Jupyter) {
    'use strict';
    return KBWidget({
        ...
        render: function () {
            ...
            var addDownloader = (descr) => {
                let $dlBtn = $('<button>').addClass('kb-data-list-btn').append(descr.name);
        ...
        downloadFile: function (urlSuffix) {
            ...
            if (self.useDynamicDownloadSupport) {
                var genericClient = new GenericClient(
                    self.srvWizURL,
                    { token: self.token },
                    null,
                    false
