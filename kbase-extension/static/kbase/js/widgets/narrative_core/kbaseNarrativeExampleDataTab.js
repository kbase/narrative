...
define([
    ...
    return KBWidget({
        ...
        getExampleDataAndRender: function () {
            if (!this.dataConfig) {
                this.showError(
                    "Unable to load example data configuration! Please refresh your page to try again. If this continues to happen, please <a href='https://www.kbase.us/support/'>click here</a> to contact KBase with the problem."
                );
                return;
            }
            ...
            if (this.narWs) {
                Promise.resolve(
                    this.serviceClient.sync_call('NarrativeService.list_objects_with_sets', [
                        {
            ...
            for (var t = 0; t < this.dataConfig.data_types.length; t++) {
                ...
                var $tc = $('<div>').append(
                    $('<div>')
                        ...
                        .append(
                            $('<div>').css({ margin: '4px', color: '#555' }).append(typeInfo.header)
                        )
                );
