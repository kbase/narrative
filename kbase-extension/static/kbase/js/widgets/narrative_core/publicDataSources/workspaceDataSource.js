define([
    ...
    './common',
], function (Promise, Handlebars, Utils, DynamicServiceClient, common) {
    'use strict';
        ...
        {},
        {
                                ...
                                .then(
                                    function () {
                                        ...
                                        this.fetchedDataCount =
                                            this.page * this.pageSize + paged.length;
            ...
            applyQuery: {
                value: function (queryExpression) {
                    ...
                    this.filteredData = this.availableData.filter(
                        function (item) {
                            return this.config.searchFields.some(
                                function (field) {
                                    var value = Utils.getProp(item, field);
                                    if (
                                        value &&
                                        value.toLowerCase().indexOf(queryExpression) >= 0
                                    ) {
                                        return true;
                                    }
                                    return false;
                            ...
                            return this.narrativeService
                                ...
                                .spread(
                                    function (data) {
                                        ...
                                        return this.availableData.sort(
                                            function (a, b) {
                                                var sortField = this.config.sort[0].field;
                                                var direction = this.config.sort[0].ascending
                                                    ? 1
                                                    : -1;
            ...
            transform: {
                value: function () {
                    return this.currentResultData.map(
                        function (item) {
                            var name = this.titleTemplate(item);
                            var metadata = common.applyMetadataTemplates(
