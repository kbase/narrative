        ...
        startingChordPosition: function (d, idx) {
            if (this.initialized) {
                if (idx < this.lastChordData.length - 1) {
                    ...
                    return {
                        source: {
                            startAngle: this.options.endAngle,
            ...
            //the first line animates the wedges in place, the second animates from the top, the third draws them rendered
                ...
                return {
                    source: {
                        ...
                        endAngle: this.options.startAngle,
                    },
                    target: {
                        startAngle: this.options.startAngle,
                        endAngle: this.options.startAngle,
        ...
        renderChart: function () {
            ...
            $.each(chordGroups, (idx, val) => {
                ...
                if ($chord.options.choppedGroups) {
                    ...
                    const color =
                        val.data.color || $chord.options.chordColorScale(idx, val.data, $chord);
                    ...
                    row.sort($chord.options.sortSubgroups).forEach((d, idx2) => {
            ...
            const fillScale = d3.scale
                .ordinal()
                .domain(d3.range(4))
                .range(['#000000', '#FFDD89', '#957244', '#F26223']);

            if (this.options.drawChords) {
                const chordG = this.data('D3svg')
                ...
                const arcMaker = d3.svg.arc().innerRadius(innerRadius).outerRadius(outerRadius);

                const newChords = [];
                //$.each(
                //    chordLayout.chords(),

                chordLayout.chords().forEach((val, idx) => {
                    //function (idx, val) {
                ...
                const funkyTown = function () {
                    this.attr('fill-opacity', 0.67)
                        .attr('fill', (d, idx) => {
                            return (
                                d.data.color ||
                                $chord.options.colorScale(d.data.colorIdx, d.data, $chord)
                            );
                        })
                        .attr('stroke', 'black')
            ...
            const tickG = this.data('D3svg')
                .select(this.region('chart'))
                .selectAll('.ticks')
                .data([0]);
            tickG.enter().insert('g', '.labelG').attr('class', 'ticks');
            tickG.attr(
                'transform',
                'translate(' +
                    (bounds.size.width / 2 + this.options.xOffset) +
            ...
            const tickArcs = tickG.selectAll('.tickArcs').data(chordLayout.groups);
            tickArcs.enter().append('g').attr('class', 'tickArcs');
