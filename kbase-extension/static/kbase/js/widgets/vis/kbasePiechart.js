...
define([
    ...
    return KBWidget({
        ...
        midPosition: function (d) {
            ...
            const ret = (0 < midAngle && midAngle < Math.PI) || midAngle < -Math.PI ? 1 : -1;
            return ret;
        },
        ...
        useOutsideLabels: function (d) {
            ...
            return (
                (this.options.outsideLabels && d.data.outsideLabel == undefined) ||
                d.data.outsideLabel
            );
        },
        ...
        outerRadius: function () {
            ...
            if (radius <= 0) {
                let diameter =
                    bounds.size.width < bounds.size.height ? bounds.size.width : bounds.size.height;
        ...
        renderChart: function () {
            ...
            const arcMaker = d3.svg.arc().innerRadius(innerRadius).outerRadius(radius);

            const textArcMaker = d3.svg
                .arc()
                .innerRadius(innerRadius + ((radius - innerRadius) * 8) / 10)
                .outerRadius(innerRadius + ((radius - innerRadius) * 8) / 10);

            const smallArcMaker = d3.svg
                .arc()
            ...
            const funkyTown = function () {
                ...
                this.attr('transform', (d, idx) => {
                    if (d.data.offset == undefined) {
                        return;
                    }

                    const sliceMover = d3.svg
                        .arc()
                        .innerRadius(0)
                    ...
                    if ($pie.options.gradient) {
                        this
                            ...
                            .attrTween('fill', (d, idx) => {
                                ...
                                const currentID =
                                    uniqueFunc == undefined ? undefined : uniqueFunc(d);

                                let gradID = d.data.gradID;
                                if (gradID == undefined) {
                                    let newGradID;
                                    if (
                                        $pie.lastPieData != undefined &&
                                        idx < $pie.lastPieData.length
                                    ) {
                ...
                if (this.attrTween) {
                    this.text((d) => {
                        ...
                        .attrTween('transform', function (d, idx) {
                            ...
                            const myArcMaker = useOutsideLabels ? textArcMaker : arcMaker;
            ...
            drag.on('dragstart', function (d) {
                ...
                .on('drag', function (d) {
                    ...
                    if (this.__delta > dragThrottle || this.__delta < -1 * dragThrottle) {
                        ...
                        const proposedStartAngle =
                            currentStartAngle + (Math.PI * (distance / 2)) / radius;
            ...
            if ($pie.options.pieColor != undefined) {
                ...
                const pieMaker = d3.svg.arc().innerRadius(0).outerRadius(radius);
            ...
            $pie.outerArc.enter().append('path').attr('class', 'outerArc');
            ...
            const transitionTime =
                this.initialized || this.options.startingPosition != 'final'
            ...
            const labels = labelG.selectAll('.label').data(
                pieData.filter((d) => {
                    return (
                ...
                if (this.attrTween) {
                    this.attrTween('stroke-opacity', function (d, idx) {
                        ...
                        return function (t) {
                            ...
                            return [
                                smallArcMaker.centroid(d2),
                                myArcMaker.centroid(d2),
                                textAnchor,
                            ];
                        };
                    });
            ...
            const lines = labelG.selectAll('polyline').data(
                pieData.filter((d) => {
                    return (
                        ($pie.options.labels || d.data.forceLabel) &&
