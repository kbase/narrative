...
define([
    ...
    return KBWidget({
        ...
        redBlue: function (node, d) {
            ...
            if ($tree.options.red != undefined && $tree.options.blue != undefined) {
                $tree.comparison(
                    'Comparing ' + $tree.options.red.name + ' vs ' + $tree.options.blue.name
                );
            } else {
                $tree.comparison('');
            }
        ...
        defaultNodeClick: function (d) {
            if (
                !this.findInChildren(this.options.red, d) &&
                !this.findInChildren(this.options.blue, d)
            ) {
                this.toggle(d);
                this.updateTree(d);
            }
        ...
        updateTree: function (source) {
            ...
            const rootText = chart
                ...
                .text(root.name);
            rootOffset =
                rootText[0][0].getBBox().width + $tree.options.labelSpace + bounds.origin.x;
            ...
            this.nodes.forEach((d) => {
                ...
                d.y =
                    $tree.options.fixed && (!d.children || d.children.length == 0)
                ...
                if ($tree.options.labelWidth && d.width > $tree.options.labelWidth) {
                    ...
                    let throttle = 0;
                    while (
                        findWidth(fakeText, d)[2] < $tree.options.labelWidth &&
                        throttle++ < 40
                    ) {
            ...
            let newWidth =
                this.options.xGutter + this.options.yGutter + widthDelta + bounds.size.width;
            ...
            const node = chart.selectAll('g.tree-node').data(this.nodes, (d) => {
            ...
            nodeEnter
                ...
                .style('fill', (d) => {
                    return d._children
                        ? $tree.options.openCircleFill
                        : $tree.options.closedCircleFill;
                })
                .on('click', function (d) {
                    if ($tree.oneClick) {
            ...
            const nodeUpdate = node
                ...
                .attr('transform', (d) => {
                    let y =
                        $tree.options.fixed && (!d.children || d.length == 0)
                            ? $tree.options.fixedDepth
                            : d.y;
            ...
            const link = chart
                ...
                .data($tree.treeLayout.links($tree.nodes), (d) => {
        ...
        renderChart: function () {
            ...
            const getYCoords = function (d) {
                ...
                if (
                    ...
                    targetY =
                        $tree.options.fixedDepth - d.target.nodeDepth * $tree.options.distance;
                    sourceY =
                        $tree.options.fixedDepth - d.source.nodeDepth * $tree.options.distance;
