...
define(['kbwidget', 'bootstrap', 'jquery', 'd3', 'kbaseBarchart', 'kbaseAuthenticatedWidget'], (
    KBWidget,
    bootstrap,
    $,
    d3,
    ...
    return KBWidget({
                ...
                for (let f = 0; f < keys.length; f++) {
                    ...
                    if (!groups[group]) {
                        const $groupButton = $.jqElem('div')
                                ...
                                $.jqElem('button')
                                    ...
                                    .on('click', function (e) {
                                        ...
                                        if ($(this).parent().hasClass('open')) {
                                            ...
                                            const $span = $(this).next().find('span');
                                            $span.toggleClass('fa-plus fa-caret-up');
                                        }

                                        const $check = $(this).parent().find('.check');
                                        const shouldOpen = $check.data('checked') ? false : true;
                                        ...
                                        if (shouldOpen) {
                                            ...
                                            $check.data(
                                                'checked',
                                                $(this).parent().find('.subsystem-checkbox').length
                                            );
                            ...
                            .append(
                                $.jqElem('ul').addClass('dropdown-menu').css({
                                    width: '450px',
                                    'padding-left': '5px',
                                    'text-align': 'left',
                                })
        ...
        init: function init(options) {
            ...
            const ws = new Workspace(window.kbconfig.urls.workspace, { token: $pmi.authToken() });
            ...
            $.when(ws.get_objects([subanno_params]), ws.get_objects([fbaobj_params]))
                .then((d1, d2) => {
            ...
            const $container = $.jqElem('div')
                ...
                .append(
                    $.jqElem('div')
                        .attr('id', 'loader')
                        .append(
                            '<br>&nbsp;Loading data...<br>&nbsp;please wait...<br>&nbsp;Data parsing may take upwards of 30 seconds, during which time this narrative may be unresponsive.'
                        )
                        .append($.jqElem('br'))
                        .append(
                            $.jqElem('div')
