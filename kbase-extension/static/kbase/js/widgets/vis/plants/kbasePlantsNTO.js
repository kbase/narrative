...
define(['kbwidget', 'bootstrap', 'jquery', 'kbaseIrisWidget', 'kbaseTable'], (
    KBWidget,
    bootstrap,
    $,
    kbaseIrisWidget,
    ...
    return KBWidget({
        ...
        appendUI: function ($elem) {
            ...
            if (this.input() == undefined) {
                ...
                $.each(this.input().nodes, (idx, cluster) => {
                ...
                $.each(this.input().edges, (eidx, edge) => {
                    $.each(datasets, (idx, dataset) => {
                        //if (throttle++ > 10) {
                        //    return false;
                        //}
                ...
                $.each(this.input().nodes, (idx, node) => {
                    $.each(edge_ids, (eidx, edge_id) => {
                        if (node.id == edge_id) {
                ...
                const linkScale = d3.scale.pow().domain([0, datasets.length]).range([-100, 100]);
                ...
                $.each(this.input().edges, (eidx, edge) => {
                    $.each(datasets, (idx, dataset) => {
                        ...
                        if (validEdge) {
                            ...
                            const edgeName = [edge.dataset_id, node1.name, node2.name]
                                .sort()
                ...
                const $checkbox = $.jqElem('input').attr('type', 'checkbox');
                $.each(datasets, (ridx, row) => {
                    const checkbox = {
                    ...
                    checkbox.setup = function ($checkbox, cell) {
                        $checkbox.on('click', function (e) {
                            ...
                            $.each(newDataset.nodes, (idx, node) => {
                                node.label =
                                    '<b>' +
                                    node.name +
                                    '</b>' +
                            ...
                            $.each(newDataset.edges, (idx, edge) => {
                                edge.label =
                                    '<b>' +
                                    edge.description +
                                    '</b>' +
