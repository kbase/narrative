...
define(['kbwidget', 'bootstrap', 'jquery', 'kbaseIrisWidget', 'kbaseTable'], function (
    KBWidget,
    bootstrap,
    $,
    kbaseIrisWidget,
    ...
    return KBWidget({
        ...
        _accessors: ['terminal', 'networkGraph'],
        ...
        appendUI: function ($elem) {
            ...
            if (this.input() == undefined) {
                ...
                $.each(this.input().nodes, function (idx, cluster) {
                    if (cluster.type == 'CLUSTER') {
                        ...
                        datasets.push({
                            ...
                            styled_cluster_id: {
                                value: cluster.entity_id,
                                style: 'color : ' + colorCats(datasets.length % 20),
                            },
                            color: colorCats(datasets.length % 20),
                            num_genes: 0,
                            num_edges: 0,
                            go_enrichment:
                                cluster.user_annotations.go_enrichnment_annotation ||
                                'No enrichment',
                            go_id: go_id,
                            p_value: p_value,
                            go_term: go_term,
                ...
                $.each(this.input().edges, function (eidx, edge) {
                ...
                $.each(this.input().nodes, function (idx, node) {
                    $.each(edge_ids, function (eidx, edge_id) {
                        if (node.id == edge_id) {
                ...
                var linkScale = d3.scale.pow().domain([0, datasets.length]).range([-100, 100]);
                ...
                $.each(this.input().edges, function (eidx, edge) {
                    $.each(datasets, function (idx, dataset) {
                        ...
                        if (
                            node_to_cluster[edge.node_id1].id == dataset.id &&
                            (node_to_cluster[edge.node_id2] || {}).id == dataset.id
                        ) {
                            dataset.num_edges++;
                            validEdge = true;
                            datasetRec = node_to_cluster[edge.node_id1];
                        } else if (
                            node_to_cluster[edge.node_id2] == dataset.id &&
                            (node_to_cluster[edge.node_id1] || {}).id == dataset.id
                        ) {
                            dataset.num_edges++;
                            validEdge = true;
                        ...
                        if (validEdge) {
                            ...
                            var edgeName = [edge.dataset_id, node1.name, node2.name]
                                .sort()
                                .join('-'); //node1.name + '-' + node2.name;
                            ...
                            if (edgeObj == undefined) {
                                edgeObj = edges[edgeName] = {
                                    ...
                                    colors: {},
                                    curveStrength:
                                        linkScale(dataset_idx) * (dataset_idx % 2 ? -1 : 1),
                                };
                            }
                ...
                var $checkbox = $.jqElem('input').attr('type', 'checkbox');
                $.each(datasets, function (ridx, row) {
                    var checkbox = {
                    ...
                    checkbox.setup = function ($checkbox, cell) {
                        $checkbox.on('click', function (e) {
                            ...
                            $.each(dataset.nodes, function (idx, node) {
                            ...
                            $.each(dataset.edges, function (idx, edge) {
                            ...
                            if ($check.checked) {
                                ...
                                $.each(row.nodes, function (idx, node) {
                                ...
                                $.each(row.edges, function (idx, edge) {
                            ...
                            $.each(newDataset.nodes, function (idx, node) {
                                node.label =
                                    '<b>' +
                                    node.name +
                                    '</b>' +
                            ...
                            $.each(newDataset.edges, function (idx, edge) {
                                edge.label =
                                    '<b>' +
                                    edge.description +
                                    '</b>' +
                    ...
                    row_callback: function (cell, header, row, $kb) {
                        if (header == 'gene_list') {
                            ...
                            for (var i = 0; i < 3 && i < row.gene_data.length; i++) {
                                ...
                                $return.append($.jqElem('li').append(label));
                            }
                        ...
                        if (header.match(/^(go_id|go_term|p_value)$/)) {
                            ...
                            for (var i = 0; i < row[header].length; i++) {
                                $return.append($.jqElem('li').append(row[header][i]));
                            }
                ...
                var $tables = $.jqElem('div')
                    .attr('id', 'tables')
                    .append($.jqElem('div').attr('id', 'cluster_table'))
                    .append($.jqElem('div').attr('id', 'gene_table'))
                    .append($.jqElem('div').attr('id', 'network'));
