...
define([
    ...
    'kbaseForcedNetwork',
    'kbaseTable',
], function (KBWidget, bootstrap, $, kbasePlantsNetworkTable, kbaseForcedNetwork, kbaseTable) {
    return KBWidget({
        ...
        setInput: function (newInput) {
            ...
            var records = {};
            var datasets = [];
            $.each(newInput.datasets, function (idx, rec) {
            ...
            $.each(newInput.nodes, function (idx, node) {
                ...
                if (nodeObj == undefined) {
                    ...
                    nodeObj = nodes[node.id] = {
                        ...
                        radius: 10,
                        tag: node.name,
                        search: [node.name, node.func, JSON.stringify(node.associations)].join(''),
            ...
            $.each(newInput.edges, function (idx, edge) {
            ...
            $.each(records, function (idx, val) {
                if (val.nodes.length) {
                    tabularData.push({
                        ...
                        dataset: {
                            value: val.dataset,
                            style: 'color : ' + colorCats(datasets.indexOf(val.dataset) % 20),
                        },
                        nodes: val.nodes,
                        edges: val.edges,
        ...
        appendUI: function ($elem) {
            ...
            $elem
                ...
                .append($msgBox)
                .append($.jqElem('div').attr('align', 'center').append($networkGraph.$elem));
