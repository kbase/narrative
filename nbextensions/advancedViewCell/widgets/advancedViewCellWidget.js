    ...
    function factory(config) {
        ...
        function buildPython(cell, cellId, app, params) {
            var runId = new Uuid(4).format(),
                fixedApp = fixApp(app),
                outputWidgetState =
                    utils.getCellMeta(cell, 'viewCell.outputWidgetState') ||
                    null,
        ...
        function start() {
            return Promise.try(function () {
                ...
                eventManager.add(
                    cellBus.on('metadata-changed', function () {
                        evaluateAppState();
                    })
                );
        ...
        function loadInputWidget() {
            return new Promise(function (resolve, reject) {
                var selectedWidget =
                    'nbextensions/advancedViewCell/widgets/appParamsWidget';
        ...
        function makeIcon() {
            ...
            var appSpec = env.appSpec,
                nmsBase = runtime.config(
                    'services.narrative_method_store_image.url'
                ),
                iconUrl = Props.getDataItem(appSpec, 'info.icon.url');

            if (iconUrl) {
                return span(
                    { class: 'fa-stack fa-2x', style: { padding: '2px' } },
                    [
                        img({
                            src: nmsBase + iconUrl,
                            style: {
