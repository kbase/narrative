define([
    ...
    function factory(config) {
        ...
        function loadParamsWidget(arg) {
            return pRequire(['./appParamsWidget']).spread(function (Widget) {
                ...
                bus.respond({
                    ...
                    handle: function () {
                        ...
                        return (
                            canDoBatch &&
                            (model.getItem('user-settings.batchMode') || false)
                        );
                    },
                });
        ...
        function loadViewParamsWidget(arg) {
            return pRequire(['./appParamsViewWidget']).spread(function (
                Widget
            ) {
                // TODO: widget should make own bus.
                var bus = runtime.bus().makeChannelBus({
        ...
        function startTab(tabId) {
            ...
            if (selectedTab.widgetModule) {
                return loadWidget(selectedTab.widgetModule).then(function (
                    Widget
                ) {
                    ...
                    ui.getElement(
                        'run-control-panel.tab-pane.widget'
                    ).appendChild(node);
            ...
            var node = document.createElement('div');
            ui.getElement('run-control-panel.tab-pane.widget').appendChild(
                node
            );
        ...
        function getAppRef() {
            ...
            if (!app || !app.spec || !app.spec.info) {
                throw new ToErr.KBError({
                    ...
                    message:
                        'This app cell is corrupt -- the app info is incomplete',
                    advice: [
        ...
        function showAboutApp() {
            ...
            ui.setContent(
                'about-app.git-commit-hash',
                appSpec.info.git_commit_hash || ui.na()
            );
            ui.setContent(
                'about-app.authors',
                (function () {
                    if (
                        appSpec.info.authors &&
                        appSpec.info.authors.length > 0
                    ) {
            ...
            var appRef = [appSpec.info.id, model.getItem('app').tag]
                    .filter(toBoolean)
                    .join('/'),
                link = a(
        ...
        function buildRunControlPanelRunButtons(events) {
            ...
            var buttonList = Object.keys(actionButtons.availableButtons).map(
                function (key) {
        ...
        function buildRunControlPanelDisplayButtons(events) {
            var buttons = Object.keys(controlBarTabs.tabs)
                .map(function (key) {
                                ...
                                div(
                                    ...
                                    [
                                        div(
                                            ...
                                            buildRunControlPanelDisplayButtons(
                                                events
                                            )
                                        ),
                                    ]
        ...
        function buildPython(cell, cellId, app, params) {
            ...
            if (
                ...
                code = PythonInterop.buildBatchAppRunner(
                    cellId,
                    runId,
                    fixedApp,
                    [params]
                ...
                code = PythonInterop.buildAppRunner(
                    cellId,
                    runId,
                    fixedApp,
                    params
        ...
        function renderUI() {
            ...
            var indicatorNode = ui.getElement(
                'run-control-panel.status.indicator'
            );
            var iconNode = ui.getElement(
                'run-control-panel.status.indicator.icon'
            );
            if (iconNode) {
                // clear the classes
                ...
                iconNode.classList.add(
                    'fa',
                    'fa-' + state.ui.appStatus.icon.type,
                    'fa-3x'
                );
            }
        ...
        function saveNarrative() {
            ...
            if (readOnly) {
                console.warn(
                    'cannot save narrative in read-only mode; save request ignored'
                );
                return;
            }
        ...
        function updateFromLaunchEvent(message) {
            ...
            var newFsmState = (function () {
                switch (message.event) {
                    ...
                    default:
                        throw new Error(
                            'Invalid launch state ' + message.event
                        );
                }
            })();
                ...
                switch (jobState.status) {
                    ...
                    case 'running':
                        ...
                        if (jobState.child_jobs && jobState.child_jobs.length) {
                            let childDone = jobState.child_jobs.some(
                                (childState) => {
                                    return (
                                        [
                                            'completed',
        ...
        function startListeningForJobMessages(jobId) {
            ...
            ev = runtime.bus().listen({
                ...
                handle: function (message) {
                    ...
                    if (
                        !existingState ||
                        !utils2.isEqual(existingState, newJobState)
                    ) {
                        model.setItem('exec.jobState', newJobState);
                        if (outputWidgetInfo) {
                            model.setItem(
                                'exec.outputWidgetInfo',
                                outputWidgetInfo
                            );
                        }
        ...
        function doOnSuccess() {
            ...
            var outputCellId = model.getItem([
                'output',
                'byJob',
                jobId,
                'cell',
            ...
            var skipOutputCell =
                model.getItem('exec.outputWidgetInfo.name') === 'no-display';
            var cellInfo;
            if (skipOutputCell) {
                cellInfo = {
                    created: false,
        ...
        function start() {
            return Promise.try(function () {
                ...
                .then(function () {
                    ...
                    busEventManager.add(
                        bus.on('actionButton', function (message) {
                    ...
                    busEventManager.add(
                        runtime
                            .bus()
                            .on('kernel-state-changed', function (msg) {
            ...
            Object.keys(params).forEach(function (key) {
                ...
                if (!paramSpec) {
                    console.error(
                        'Parameter ' +
                            key +
                            ' is not defined in the parameter map',
                        parameters
                    );
                    throw new Error(
                        'Parameter ' +
            ...
            function harvestErrors(validations) {
                if (validations instanceof Array) {
                    validations.forEach(function (result, index) {
                        if (!result.isValid) {
                            messages.push(
                                String(index) + ':' + result.errorMessage
                            );
                        }
        ...
        function checkSpec(appSpec) {
            ...
            if (!cellAppSpec) {
                throw new ToErr.KBError({
                    ...
                    message:
                        'This app cell is misconfigured - it does not contain an app spec',
        ...
        function run() {
            ...
            return Promise.try(function () {
                ...
                .then(function (appSpec) {
                    ...
                    if (
                        appSpec &&
                        utils.getCellMeta(cell, 'kbase.type') !== 'devapp'
                    ) {
