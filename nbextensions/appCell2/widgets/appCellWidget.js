define([
    ...
    function factory(config) {
        ...
        function doStartRunning() {
            ...
            var exec_start_time = new Date(jobState.running).getTime();
        ...
        function doStartQueueing() {
            ...
            var creation_time = new Date(jobState.created).getTime();
            widgets.runClock.start({
                node: ui.getElement('run-control-panel.status.execMessage.clock'),
            ...
            widgets.runClock = RunClock.make({
                on: {
                    tick: function(elapsed) {
                        ...
                        if (elapsed > day) {
                            var finish_time = new Date(jobState.finished).getTime();
                            clock = span([
                                ' on ',
            ...
            widgets.runClock = RunClock.make({
                on: {
                    tick: function(elapsed) {
                        ...
                        var day = 1000 * 60 * 60 * 24;
                        if (elapsed > day) {
                            var finish_time = new Date(jobState.finisheds).getTime();
                            clock = span([
                                ' on ',
                                format.niceTime(finish_time)
                            ]);
