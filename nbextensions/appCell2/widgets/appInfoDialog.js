...
define([
...
], function (Promise, Runtime, html, UI, Catalog, NarrativeMethodStore) {
    'use strict';
    ...
    function render(data) {
        ...
        return div({ class: 'row' }, [
            div(
                { class: 'col-md-8' },
                div({ class: 'kb-app-cell-info-desc' }, [
                    div(
                        {
                            style: {
                                maxHeight: '400px',
            ...
            div(
                ...
                div({ class: 'kb-app-cell-info' }, [
                    ...
                    div([
                        'Updated: ',
                        span({ class: 'value' }, data.updateDate),
                    ]),
                    div([
                        'Run count: ',
                        span({ class: 'value' }, data.runCount),
                    ]),
                ])
            ),
        ]);
    ...
    function show(arg) {
        ...
        return Promise.all([
            ...
            .spread(function (methodInfo, aggregateStats, moduleInfo) {
                ...
                var title = [methodInfo[0].name].join(''),
                    info = {
                        id: arg.id,
                        tag: arg.tag,
                        version:
                            arg.tag === 'release'
                                ? methodInfo[0].ver
                                : methodInfo[0].git_commit_hash.substr(0, 7),
                        description: methodInfo[0].description || '',
                        authors: methodInfo[0].authors || [],
                        runCount: aggregateStats[0]
                            ? aggregateStats[0].number_of_calls
                            : 'n/a',
                        updateDate: new Date(
                            moduleInfo[arg.tag].timestamp
                        ).toLocaleDateString(),
                    },
                    url = [
