define([
    ...
    'kb_common/html',
    './jobStateListRow',
], function (Promise, Runtime, UI, format, html, JobStateListRow) {
    'use strict';
            ...
            return Promise.try(function () {
                ...
                return Promise.try(() => {
                    ...
                    container
                        ...
                        .classList.add('job-selected'); // start with the parent selected

                    for (
                        var i = 0;
                        i < Math.max(arg.batchSize, arg.childJobs.length);
                        i++
                    ) {
                        var jobId = null,
                            initialState = null;
                        ...
                        createJobStateWidget(
                            ...
                            initialState,
                            arg.clickFunction
                        ); // can make null ones. these need to be updated.
                    }
                }).then(() => {
                    startParentListener();
                });
