define([
    ...
    function factory(config) {
        ...
        function makeFieldWidget(appSpec, parameterSpec, value) {
            var fieldBus = runtime
                    .bus()
                    .makeChannelBus({ description: 'A field widget' }),
        ...
        function renderParameters() {
            ...
            return Promise.try(function () {
                var params = validateParameterSpecs(
                    model.getItem('parameters')
                );
                ...
                return Promise.resolve()
                    .then(function () {
                        if (params.length === 0) {
                            ...
                            return Promise.all(
                                params.map(function (spec) {
                                    try {
                                        var result = makeFieldWidget(
                                                appSpec,
                                                spec,
                                                model.getItem([
                                                    'params',
                                            ...
                                            rowNode = document.createElement(
                                                'div'
                                            );
                                        // places.fields.appendChild(rowNode);
                                        var rowPlace = document.querySelector(
                                            '[data-parameter="' +
                                                spec.name() +
                                                '"]'
                                        );
                                        ...
                                        places.fields.appendChild(
                                            ui.createNode(errorDisplay)
                                        );
                                    }
                                })
                            );
