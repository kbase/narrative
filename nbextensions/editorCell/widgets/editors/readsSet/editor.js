define([
    ...
    function factory(config) {
        var cell = config.cell,
            ...
            bus = runtime
                .bus()
                .makeChannelBus({ description: 'An editor cell widget' }),
            env = {},
            editorState,
            model,
        ...
        function syncAppSpec(appId, appTag) {
            ...
            return nms.get_method_spec(appRef).then(function (data) {
                if (!data[0]) {
        ...
        function syncFatalError() {
            ui.setContent(
                'fatal-error.title',
                editorState.getItem('fatalError.title')
            );
            ui.setContent(
                'fatal-error.message',
                editorState.getItem('fatalError.message')
            );
            ui.setContent(
                'fatal-error.details',
                editorState.getItem('fatalError.details')
            );
            ui.setContent(
                'fatal-error.advice',
                editorState.getItem('fatalError.advice')
            );
        }
        ...
        function renderSetting(settingName) {
            ...
            value = editorState.getItem(
                ['user-settings', settingName],
        ...
        function buildPython() {
            ...
            var code = PythonInterop.buildEditorRunner(
                cellId,
                runId,
                fixedApp,
                params
        ...
        function showWidget(name, widgetModule, path) {
            var bus = runtime
                    .bus()
                    .makeChannelBus({ description: 'Bus for showWidget' }),
        ...
        function renderUI() {
            ...
            ui.setContent(
                'flags.fsm',
                JSON.stringify(fsm.getCurrentState().state)
            );
        }
        ...
        function loadUpdateEditor(controller) {
            return new Promise(function (resolve, reject) {
                require(['./update'], function (Widget) {
                    ...
                    bus.on('parameter-touched', function () {
                        var state = fsm.getCurrentState(),
        ...
        function renderCurrentlyEditing() {
            ...
            var content = table({ class: 'table table-striped' }, [
                tr([
                    th('Name'),
                    td({ style: { fontWeight: 'bold' } }, info.name),
                ]),
        ...
        function loadData(objectRef) {
            ...
            return setApiClient
                .callFunc('get_reads_set_v1', [params])
                .spread(function (setObject) {
                    ...
                    model.setItem(
                        'params.description',
                        setObject.data.description
                    );
                    model.setItem(
                        'params.items',
                        setObject.data.items.map(function (item) {
                            return {
                                ref: item.ref,
                                label: item.label,
                            };
                    ...
                    var objectInfo = serviceUtils.objectInfoToObject(
                        setObject.info
                    );
        ...
        function doCreateNewSet(name) {
            ...
            return setApiClient
                ...
                .spread(function (result) {
                    doEditObject(
                        serviceUtils.objectInfoToObject(result.set_info)
                    );
                })
                .catch(function (err) {
                    console.error('ERROR getting reads set ', err);
                    console.error(
                        err.detail ? err.detail.replace('\n', '<br>') : ''
                    );
                    loadErrorWidget(err);
                });
        }
        ...
        function unloadEditorController(controller) {
            ...
            return Promise.try(function () {
                ...
                if (controller.widget) {
                    ...
                    return controller.widget.stop().then(function () {
        ...
        function unloadEditors() {
            return Promise.all(
                Object.keys(editorControllers).map(function (id) {
                    var controller = editorControllers[id];
                    return unloadEditorController(controller).then(function () {
        ...
        function unloadWidgets() {
            return Promise.all(
                Object.keys(editorControllers).map(function (id) {
                    var controller = editorControllers[id];
                    return unloadWidgetController(controller).then(function () {
                        delete editorControllers[id];
                    });
                })
            );
        ...
        function loadEditObjectSelector() {
            return new Promise(function (resolve, reject) {
                require(['./selector'], function (Widget) {
                    ...
                    widget.channel.on('changed', function (message) {
                        ...
                        var currentObject = editorState.getItem(
                            'current.set.info'
                        );
        ...
        function evaluateAppState() {
            return Promise.try(function () {
                var currentEditorSetInfo = editorState.getItem(
                    'current.set.info'
                );
                ...
                return validateModel().then(function (result) {
        ...
        function registerEvents() {
            ...
            eventManager.add(
                cellBus.on('result', function (message) {
                    ...
                    if (
                        ...
                        setStatus(
                            div([
                                p(
                                    'Error! result message is not from the generated code!'
                                ),
                                p([
                                    'Sent ',
                                    editorState.getItem(
                    ...
                    if (message.message.result) {
                        ...
                        editorState.setItem(
                            'current.set.ref',
                            message.message.result.set_ref
                        );
                        var objectInfo = serviceUtils.objectInfoToObject(
            ...
            return loadSpec()
                ...
                .then(function () {
                    ...
                    if (editorState.getItem('current.set.info')) {
                        ...
                        return doEditObject(
                            editorState.getItem('current.set.info')
                        );
                    } else {
                        return doShowWidget(MessageWidget, {
                            content: div(
                ...
                .catch(function (err) {
                    ...
                    addNotification(
                        'Error loading main widgets: ' + err.message
                    );
                    setFatalError({
