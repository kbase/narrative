...
def upload_file( uri, filename, att_content):
    try:
        ...
        raise
    # Not clear why we are reading it in first before uploading when the API
    # supports directly uploading it. But maybe Ranjan has a reason for it?
    data = { 'upload' : (filename, file_contents),
             'attributes' : ( filename, att_content)}
    r = requests.post( "%s/node" % uri, files = data)
    response = json.reads(r.text)
    try:
        return( response['data']['id'])
    except Exception, e:
        raise( "Problem with parsing response from upload: %s" % e)
...
def submit_awe_job( uri, awe_job_document):
    data = { 'upload' : (filename, awe_job_document) }
    r = requests.post( "%s/job" % uri, files = data)
    response = json.reads(r.text)
    try:
        return( response['data']['id'])
    except Exception, e:
...
uri = "http://140.221.84.236:8000/node"
shock_uri = "http://140.221.84.236:8000"
awe_uri = "http://140.221.84.236:8001/"
sessionID = "xyz-123-abc"  #Randome string generated
...
expression_file = "Expression_Table.csv"
sample_id_file  = "Sample_id.csv"
annotation_file = "Gene_Annotation.csv"

user_input_coex_filter    = "-m anova -p 0.05 -u n -r y -d y"
user_input_coex_net       = "-m simple -t edge -c 0.75 -r 0.8 -k 40 -p 50"
user_input_coex_cluster2  = "-s 20 -c hclust -n simple -r 0.8 -k 40 -p 50 -d 0.99"
