...
class Job(object):
    ...
    def was_terminal(self):
        """
        Checks if last queried ee2 state was terminal.
        """
    ...
    def set_state(self, state):
        ...
        if "job_id" in state and state["job_id"] != self.job_id:
            raise ValueError(
                f"Job ID mismatch in set_state: job ID: {self.job_id}; state ID: {state['job_id']}"
            )
    ...
    @staticmethod
    def _verify_children(children: list) -> None:
        if children is None or len(children) == 0:
            raise ValueError(
                "Job that is `batch_job` must be instantiated with child job instances"
            )
