...
class JobManager:
    ...
    def _construct_job_output_state_set(
        ...
        {
            "job_id": string,
            "jobState": dictionary containing job data from EE2,
            "outputWidgetInfo": dictionary | None
            "batch_id": string if present; otherwise None,
        }

        If the call to refresh job data from ee2 fails, the job output state will have an additional
        'error' key with the error message from the failed data refresh
        {
            "job_id": string,
            ...,
