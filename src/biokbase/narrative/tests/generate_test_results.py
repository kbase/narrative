...
It uses data from `ee2_job_test_data_file` and `app_specs_file` and provides expected narrative backend message data,
as well as a number of mappings that are used in python tests.

The narrative backend message data is written as JSON to the `response_data_file`, indexed by message type and job ID.
By default, if `response_data_file` does not exist when `generate_test_results.py` is run, job message data will be saved there.
The `response_data_file` can also be generated by running `generate_test_results.py` with the `--force` argument.
"""
