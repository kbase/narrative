...
class NarrativeRefTestCase(unittest.TestCase):
    ...
    def test_no_objid_fail(self):
        ...
        self.assertIn(
            "Couldn't find Narrative object id in Workspace metadata", str(e.exception)
        )
    ...
    def test_ref_init_fail(self):
        ...
        self.assertIn(
            "A numerical Workspace id is required for a Narrative ref, not x",
            str(e.exception),
        )
        ...
        self.assertIn(
            "If ver is present in the ref, it must be numerical, not x",
            str(e.exception),
        )
