...
class NarrIOTestCase(unittest.TestCase):
    ...
    def test_write_narrative_bad_ref(self):
        ...
        self.assertEqual(
            "write_narrative must use a NarrativeRef as input!", str(err.exception)
        )
        self.logout()
    ...
    def test_narrative_permissions_bad(self):
        ...
        self.assertEqual(
            "narrative_permissions must use a NarrativeRef as input!",
            str(err.exception),
        )
    ...
    def test__validate_nar_type_ok(self):
        ...
        self.assertIsNone(
            self.mixin._validate_nar_type("KBaseNarrative.Narrative", None)
        )
    ...
    def test__validate_nar_type_fail(self):
        ...
        self.assertIn(
            "Expected a Narrative object, got a {}".format(bad_type), str(err.exception)
        )

        with self.assertRaises(HTTPError) as err:
            self.mixin._validate_nar_type(bad_type, ref)
        self.assertIn(
            "Expected a Narrative object with reference {}, got a {}".format(
                ref, bad_type
            ),
            str(err.exception),
        )
