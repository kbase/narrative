    ...
    def administer(self, params):
        ...
        if cmd == "listObjects":
            ...
            if (
                new_perm is None
                or new_perm == "n"
                and user_id in self.fake_ws_db[str(ws_id)]["perms"]
            ):
...
class TestWSInfoFix(unittest.TestCase):
    ...
    def test_parse_args(self):
        ...
        bad_args_set = [
            [
                ["-t", token, "-a", auth_url],
                "ws_url - the Workspace service endpoint - is required!",
            ],
            [
                ["-a", auth_url, "-w", ws_url],
                "token - a valid Workspace admin auth token - is required!",
            ],
            [
                ["-t", token, "-w", ws_url],
                "auth_url - the Auth service endpoint - is required!",
            ],
        ]
    ...
    @mock.patch(
        "src.scripts.fix_ws_metadata.fix_workspace_info.requests.get",
