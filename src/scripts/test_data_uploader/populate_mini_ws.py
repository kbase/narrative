...
def create_user(user_id):
    ...
    if r.status_code != 200 and r.status_code != 400:
        ...
        r.raise_for_status()
    r = requests.post(
        mini_auth_url + "/api/V2/testmodeonly/token",
        headers=headers,
        data=json.dumps({"user": user_id, "type": "Login"}),
    )
    ...
    ws.register_typespec(
        {
            "spec": old_spec,
            "dryrun": 0,
            "new_types": ["Narrative", "Cell", "Worksheet", "Metadata"],
        }
    )
...
def _load_workspace_data(ws, ws_data, idx, narrative_ver):
    ...
    if len(narratives):
        ...
        if len(ws_meta):
            ws.alter_workspace_metadata({"wsi": {"id": ws_id}, "new": ws_meta})
