...
def main(args):
    ...
    cookie_string = "un%3D{}%7Ckbase_sessionid%3D{}%7Cuser_id%3D{}%7Ctoken%3D{}".format(
        res["user_id"], res["kbase_sessionid"], res["user_id"], formatted_token
    )
    # 2. test unauthorized
...
def parse_args():
    ...
    p.add_argument(
        ...
        help="User to use for authenticated test (default=%(default)s)",
    )
    p.add_argument(
        "-p",
        "--password",
        dest="pw",
        default=None,
        help="Test user password -- REQUIRED",
    )
    p.add_argument(
        "-w",
        "--wronguser",
        dest="wrong_user",
        default=DEFAULT_WRONG_USER,
        help="User for attempting disallowed shutdown (default=%(default)s)",
    )
    p.add_argument(
        "-H",
        "--hosturl",
        ...
        help="Name of host to test on (e.g. narrative-dev) (default=%(default)s)",
    )
