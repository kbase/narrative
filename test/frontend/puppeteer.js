...
const check_widget_status = async (page, host, url, widget_config, token) => {
    ...
    const selector =
        '.narrative-side-panel-content > .kb-side-tab:first-child .kb-side-separator:first-child .narrative-card-row ' +
        widget_config.dataSelector;
    ...
    try {
        ...
        await page.waitForSelector(widgetDivSelector + ' ' + widget_config.widgetSelector, {
            visible: true,
        });
    ...
    if (widget_config.testFile) {
        ...
        try {
            ...
            return widgetTest({ page, host, url, widget_config, token });
        } catch (e) {
...
const run_narrative_test = async (config) => {
    ...
    Object.keys(config.widgets).forEach(async (widget_name) => {
        ...
        puppeteer
            .launch({ headless: !argv.headed, ignoreHTTPSErrors: true })
            .then(async (browser) => {
                try {
                    ...
                    console.info(
                        `      Widget status for ${config.users.userA.id} is ${
                            narrative1_widget_status ? 'good' : 'bad'
                        }`
                    );
                    ...
                    console.info(
                        `      Widget status for ${config.users.userB.id} is ${
                            narrative2_widget_status ? 'good' : 'bad'
                        }`
                    );

                    overall_status =
                        overall_status && narrative1_widget_status && narrative2_widget_status
                            ? 0
                            : 1;
                } finally {
