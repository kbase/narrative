...
define(['jquery', 'bluebird', 'kbase/js/widgets/appInfoPanel'], function ($, Promise, InfoPanel) {
    'use strict';
    ...
    describe('Test the App Info Panel module', function () {
        ...
        it('Can render with "start" and return a Promise', function (done) {
            var panel = makeDummyPanel();
            var myNode = $('<div>');
            panel.start({ node: myNode }).then(function () {
                expect(myNode.find('.kb-app-cell-info-desc p:first-child').text()).toEqual(
                    'This is a KBase wrapper for MEGAHIT.'
                );
                done();
            });
        });
