define([
    'common/cellUtils', 'narrativeMocks'
], (Utils, Mocks) => {
    'use strict';

    fdescribe('Cell utils tests', () => {
        it('has expected functions', () => {
            const fns = ['getMeta', 'setMeta', 'getCellMeta', 'setCellMeta', 'getTitle', 'findById'];
            fns.forEach((fn) => {
        ...
        describe('the getMeta function', () => {
            [{
                ...
                it(`works with ${testCase.label}`, () => {
                    ...
                    expect(Utils.getMeta(cell, testCase.group, testCase.name)).toEqual(testCase.expected);
                });
            });
