    ...
    describe('The DialogMessages module', () => {
        ...
        describe('generateDialogArgs', () => {
            ...
            it('generates appropriate strings for rerunning jobs', () => {
                ...
                tests.forEach((test) => {
                    ...
                    if (test.statusList.includes('error')) {
                        expect(result.body).toMatch(/Please note that jobs are rerun using the same parameters/)
                    }
                    else {
                        expect(result.body).not.toMatch(/Please note that jobs are rerun using the same parameters/)
                    }
                })
            })
            ...
            it('generates the appropriate string for terminating a batch job', () => {
                ...
                expect(result.body).toMatch(/Canceling the job will halt the processing of all jobs in the batch./)
            })
        })
    });
});
